#!/bin/bash
# CHORD3 cells — the to-do arrow-chord reorder vector, run ON THE GUEST on a
# routed golden-v4h clone (helpers installed + granted, `helpers-enabled true`).
#
...
beep reset; beep mark "chord3 start"
...
echo "############################################################"
echo "# CHORD3 — the to-do arrow-chord reorder vector, routed guest"
echo "# clock: $(date)"
echo "############################################################"
...
echo "===== BEEPS ====="
########################################################################
beep report 2>/dev/null | sed 's/^/     | /'
tail -n 60 "$DEPUTY_LOG" 2>/dev/null | grep -i 'reject\|error' | sed 's/^/     deputy| /'

echo ""
echo "############################################################"
if [ "$FAILURES" -eq 0 ]; then
  echo "# CHORD3 cells GREEN"
else
  echo "# CHORD3 cells RED — $FAILURES failure(s)"
fi
echo "############################################################"
exit "$FAILURES"
