#!/bin/bash
# Stage 5 cells — THE v0.20.11 BATCH. Run ON THE GUEST, on a routed golden-v4h
...
#
# The v0.20.9 list (lab/guest/stage5-cells.sh) and the v0.20.10 list
# (lab/guest/stage5-cells-0.20.10.sh) are kept verbatim beside this one: a cell
# list is the audit trail of what a release certified.
#
# This batch inherits v0.20.10's 33 cells unchanged and adds CELL 12 — DRPLC1
# (#730), the closed-loop drop slot. `ui-drag.ts` is the ONLY gate-path file the
# batch touched (`ui.ts` did not change, so CELL 7's make-repeating quadrants
# ride along as inherited coverage rather than as a required re-certification),
# and what changed in it is the drop itself: `jxaSidebarLiveDragScript` steers
# against the app's own landing placeholder with the button held, and Escapes
# rather than dropping blind. So CELL 12 rebuilds the #729 fixture — twelve
# areas, the SOURCE first carrying an expanded project group, a 26-project wall
# mid-list — and drives --last (the reported placement), --first, --before,
# --after, the SBCOL1 fold, restore-on-abort, the SPARSE=0 A/B and a
# no-scroll-bar --last through it.
#
# Every sidebar move in CELLS 11 and 12 additionally asserts the new live-drag
# record: `sidebar-drop-target` with `stop=placed`, carrying `liveVsStatic` —
# the number the maintainer's own re-run of #729 will be read off.
#
# Usage: stage5-cells-0.20.11.sh <node-binary> <app-dir>
set -u
NODE="$1"
...
# DRPLC1 (#730): every `sidebar-drop-target` record in a trace, one JSON per
# line. This is the LIVE-DRAG record — the gesture reports the point the old
# arithmetic would have aimed at (`correctedY`), the point the app's own landing
# slot put it at (`liveY`), the difference (`liveVsStatic`), and how the closed
# loop ended (`stop`, which is `placed` on every successful drop and names the
# refusal otherwise). A drop that happened without one of these did not go
# through the new gesture at all.
drop_records() {
  local tr="$1"
  [ -n "$tr" ] || return 0
  python3 -c "
...
for line in open(sys.argv[1], errors='replace'):
    ...
    if d.get('phase') == 'sidebar-drop-target': print(json.dumps(d))
" "$tr"
}
...
" "$rec"
  ...
  # DRPLC1: the live-drag record. `SB_EXPECT_DROP=0` for a cell that must refuse
  # BEFORE the gesture (an unresolvable anchor), where no drop record is owed.
  local drops ndrops nplaced ndrag
  drops=$(drop_records "$tr")
  SB_DROPS="$drops"
  ndrops=$(printf '%s\n' "$drops" | grep -c . || true)
  # PARSE the record; do not pattern-match it. `json.dumps` writes `"stop":
  # "placed"` WITH the space, so a grep for the unspaced form finds nothing and
  # calls every landed drop a failure.
  nplaced=$(printf '%s\n' "$drops" | python3 -c "
import json, sys
n = 0
for line in sys.stdin:
    ...
    if d.get('stop') == 'placed': n += 1
print(n)
")
  ndrag=$(cost_field "$rec" "g.drag")
  if [ "${ndrops:-0}" -ge 1 ]; then
    printf '%s\n' "$drops" | python3 -c "
import json, sys
for line in sys.stdin:
    line = line.strip()
    ...
    print('     drop-target: anchor=%s ordinal=%s static=%s corrected=%s span=%s live=%s liveVsStatic=%s stop=%s iters=%s axCalls=%s realized=%s' % (
        d.get('anchor'), d.get('anchorOrdinal'), d.get('staticY'), d.get('correctedY'),
        d.get('span'), d.get('liveY'), d.get('liveVsStatic'), d.get('stop'),
        len(its), d.get('axCalls'), d.get('realized')))
    for it in its[:6]:
        print('       | %s' % json.dumps(it))
"
  fi
  # A move whose area is ALREADY where it was asked for sends no gesture at all,
  # so it owes no drop record — the drag count, not the exit code, says which
  # kind of move this was.
  if [ "${SB_EXPECT_DROP:-1}" != "1" ]; then
    :
  elif [ "${ndrag:-0}" -eq 0 ]; then
    pass "[$STEP] $name — no drag was sent (nothing to move); no live-drag record owed"
  elif [ "${nplaced:-0}" -ge 1 ]; then
    pass "[$STEP] $name — live-drag record present (${ndrops} drop-target, ${nplaced} stop=placed, ${ndrag} drag gesture(s))"
  else
    fail "[$STEP] $name — ${ndrag} drag gesture(s) but no drop-target record with stop=placed (${ndrops} records)"
  fi
  if [ -n "$tr" ] && grep -q '"phase":"ui-observer".*"transport":"deputy"' "$tr"; then
...
########################################################################
echo ""
echo "===== CELL 12 — DRPLC1 (#730): the closed-loop drop slot, on the #729 shape ====="
echo "  ui-drag.ts is the only gate-path file this batch touched, and the drop"
echo "  itself is what changed: jxaSidebarLiveDragScript holds the button, reads"
echo "  the app's own landing placeholder out of the live frames, steers onto the"
echo "  slot the move asked for and only then lets go — Escape-aborting rather"
echo "  than dropping blind. So the fixture is rebuilt to #729's own shape and"
echo "  every placement is driven through it."
########################################################################

# --- 12.0 the #729 fixture: 12 areas, source FIRST with a group, wall mid-list
echo ""
echo "--- 12.0  seeding the #729 fixture (synthetic, ordinary CLI) ---"
# Clear what the earlier cells left, so the list is exactly twelve and the
# SOURCE really is the first row. That matters: DRPLC1 §3 measured to-last as
# the ONE placement whose aim lies outside the collapsed content (8 pt past the
# last row's bottom edge, where the app's answer is a courtesy rather than
# geometry), and it is the placement #729 reports.
D_UUIDS=$(db "SELECT COALESCE(group_concat(uuid,' '),'') FROM TMArea")
for U in $D_UUIDS; do
  things area delete "$U" --dangerously-permanent --allow-non-empty --json >/dev/null 2>&1
done
sleep 2
echo "     areas after clear: $(db 'SELECT count(*) FROM TMArea')"
for i in 01 02 03 04 05 06 07 08 09 10 11 12; do
  things area add "$RCTAG-D$i" >/dev/null 2>&1 || fail "12.0 could not add $RCTAG-D$i"
done
sleep 2
# Freshly made areas all carry index 0, so which NAME lands first is a uuid
# tie-break and not ours to choose (DRPLC1 seed_fixture). Read the order and
# name the roles positionally from it.
D_ORDER="$(area_order)"
D_SRC=$(python3 -c "
import sys
print(sys.argv[1].split('|')[0])
...
import sys
o = sys.argv[1].split('|')
print(o[7] if len(o) > 7 else o[-1])
" "$D_ORDER")
echo "     order after seed: $D_ORDER"
echo "     roles: SRC=$D_SRC (first, gets the expanded group)  BLOCK=$D_BLOCK (the wall)"
for i in 1 2 3; do
  things project add "$RCTAG-DPS$i" --area "$D_SRC" >/dev/null 2>&1
done
for i in $(seq 1 26); do
  things project add "$RCTAG-DPW$i" --area "$D_BLOCK" >/dev/null 2>&1
done
sleep 2
echo "     areas: $(db 'SELECT count(*) FROM TMArea')  projects: $(db 'SELECT count(*) FROM TMTask WHERE type=1 AND trashed=0')  db-modelled sidebar rows: $(sidebar_rows_db)"
osascript -e 'tell application "Things3" to activate' >/dev/null 2>&1
sleep 2
# `window 1` is NOT the main window — on this guest it addresses a 40x40
# utility window, so both the set and the read above CELL 11 silently miss and
# the fixture keeps whatever size the app opened at. Address the standard
# window the way DRPLC1's driver does.
setwin() {
  osascript -e "tell application \"System Events\" to tell process \"Things3\" to set size of (first window whose subrole is \"AXStandardWindow\") to {$1, $2}" >/dev/null 2>&1
  sleep 2
}
getwin() {
  osascript -e 'tell application "System Events" to tell process "Things3" to get size of (first window whose subrole is "AXStandardWindow")' 2>/dev/null
}
setwin 935 684
echo "     Things standard window: $(getwin)"
D_SEED_AREAS=$(db 'SELECT count(*) FROM TMArea')
D_SEED_DIGEST=$(db "SELECT COALESCE(group_concat(u,','),'') FROM (SELECT uuid||':'||COALESCE(area,'') AS u FROM TMTask WHERE trashed=0 ORDER BY uuid)" | shasum | cut -c1-12)
echo "     invariant seed: areas=$D_SEED_AREAS assignment-digest=$D_SEED_DIGEST"

# --- 12.1  THE #729 SHAPE ----------------------------------------------------
echo ""
echo "--- 12.1  --last, from the FIRST area of twelve, across the wall (#729) ---"
D_SRC_U=$(db "SELECT uuid FROM TMArea WHERE title='$D_SRC' ORDER BY \"index\", uuid LIMIT 1")
sb_move "12-1-729-last" 0 "" -- area reorder "$D_SRC_U" --last --dangerously-drive-gui --verify-timeout 240000
if [ "$(last_area "$SB_AFTER")" = "$D_SRC" ]; then
  pass "12.1 \"$D_SRC\" landed LAST — the #729 placement"
else
  fail "12.1 \"$D_SRC\" is NOT last — after: $SB_AFTER"
fi
D_FOLDED=$(python3 -c "
import json,sys
try: print(len((json.loads(sys.stdin.read()).get('data') or {}).get('collapsed') or []))
...
" <<<"$SB_OUT")
D_CHEV=$(cost_field "$SB_COST" "g.chevron")
if [ "${D_FOLDED:-0}" -ge 1 ] || [ "${D_CHEV:-0}" -ge 1 ]; then
  pass "12.1 the SBCOL1 fold rung fired (collapsed=${D_FOLDED}, chevron gestures=${D_CHEV})"
else
  fail "12.1 no fold happened — the wall is not a wall (collapsed=${D_FOLDED}, chevron=${D_CHEV})"
fi
# Things writes its group-container preferences ASYNCHRONOUSLY, so a single
# read three seconds after the drive can still see the fold that the app has
# already undone (measured: the very next move censuses all 65 rows and folds
# again, which it could only do from an expanded list). Wait for the write.
D_NC=1
...
if [ "${D_NC:-0}" -eq 0 ]; then
  pass "12.1 the disclosure state is RESTORED (collapsedAreaUUIDs empty)"
else
  fail "12.1 the sidebar is left folded — collapsedAreaUUIDs holds ${D_NC}: $(collapsed_uuids | tr '\n' ' ')"
fi

# --- 12.2  --first -----------------------------------------------------------
echo ""
echo "--- 12.2  --first: the LAST area to the top ---"
D_M2=$(last_area "$(area_order)")
D_M2_U=$(db "SELECT uuid FROM TMArea WHERE title='$D_M2' ORDER BY \"index\", uuid LIMIT 1")
sb_move "12-2-first" 0 "" -- area reorder "$D_M2_U" --first --dangerously-drive-gui --verify-timeout 240000
[ "$(first_area "$SB_AFTER")" = "$D_M2" ] \
  && pass "12.2 \"$D_M2\" is first" || fail "12.2 \"$D_M2\" did not land first — after: $SB_AFTER"

# --- 12.3  --before a mid-list anchor ---------------------------------------
echo ""
echo "--- 12.3  --before a mid-list anchor ---"
D_O="$(area_order)"
D_MOVER3=$(python3 -c "
import sys
...
D_ANCH3=$(python3 -c "
import sys
print(sys.argv[1].split('|')[6])
" "$D_O")
D_MOVER3_U=$(db "SELECT uuid FROM TMArea WHERE title='$D_MOVER3' ORDER BY \"index\", uuid LIMIT 1")
D_ANCH3_U=$(db "SELECT uuid FROM TMArea WHERE title='$D_ANCH3' ORDER BY \"index\", uuid LIMIT 1")
sb_move "12-3-before" 0 "" -- area reorder "$D_MOVER3_U" --before "$D_ANCH3_U" --dangerously-drive-gui --verify-timeout 240000
python3 -c "
import sys
o = sys.argv[1].split('|')
print('ok' if o.index(sys.argv[2]) + 1 == o.index(sys.argv[3]) else 'BAD')
" "$SB_AFTER" "$D_MOVER3" "$D_ANCH3" | grep -q ok \
  && pass "12.3 \"$D_MOVER3\" sits immediately above \"$D_ANCH3\"" \
  || fail "12.3 the --before placement did not land — after: $SB_AFTER"

# --- 12.4  --after a mid-list anchor ----------------------------------------
echo ""
echo "--- 12.4  --after a mid-list anchor ---"
D_O="$(area_order)"
D_MOVER4=$(python3 -c "
import sys
print(sys.argv[1].split('|')[1])
...
D_ANCH4=$(python3 -c "
import sys
print(sys.argv[1].split('|')[8])
" "$D_O")
D_MOVER4_U=$(db "SELECT uuid FROM TMArea WHERE title='$D_MOVER4' ORDER BY \"index\", uuid LIMIT 1")
D_ANCH4_U=$(db "SELECT uuid FROM TMArea WHERE title='$D_ANCH4' ORDER BY \"index\", uuid LIMIT 1")
sb_move "12-4-after" 0 "" -- area reorder "$D_MOVER4_U" --after "$D_ANCH4_U" --dangerously-drive-gui --verify-timeout 240000
python3 -c "
import sys
o = sys.argv[1].split('|')
print('ok' if o.index(sys.argv[2]) == o.index(sys.argv[3]) + 1 else 'BAD')
" "$SB_AFTER" "$D_MOVER4" "$D_ANCH4" | grep -q ok \
  && pass "12.4 \"$D_MOVER4\" sits immediately below \"$D_ANCH4\"" \
  || fail "12.4 the --after placement did not land — after: $SB_AFTER"

# --- 12.5  restore-on-abort --------------------------------------------------
echo ""
echo "--- 12.5  restore-on-abort: an anchor that does not exist ---"
# NOT by backgrounding Things: the drive's first step is to bring Things
# forward, so that is something it fixes rather than refuses (DRPLC1 abort_cell).
# An unresolvable anchor is the deterministic CLI-shaped abort, and the property
# it proves is the one the whole recovery ladder rests on — nothing is posted.
clear_banners
STEP=$((STEP + 1)); beep mark "[$STEP] 12-5-abort"
idx_vector() { db "SELECT COALESCE(group_concat(t,' '),'') FROM (SELECT title||'='||\"index\" AS t FROM TMArea ORDER BY \"index\", uuid)"; }
D_IDX_BEFORE=$(idx_vector)
D_AB_MOVER_U=$(db "SELECT uuid FROM TMArea ORDER BY \"index\", uuid LIMIT 1")
D_T0=$(python3 -c 'import time;print(int(time.time()*1000))')
D_AB_OUT=$(env THINGS_API_TRACE=1 "$NODE" "$APP" area reorder "$D_AB_MOVER_U" --before "$RCTAG-NO-SUCH-AREA" --dangerously-drive-gui --verify-timeout 120000 --json 2>/dev/null)
D_AB_CODE=$?
D_T1=$(python3 -c 'import time;print(int(time.time()*1000))')
D_IDX_AFTER=$(idx_vector)
printf '%s\n' "$D_AB_OUT" >"$OUT/12-5-abort.json"
echo "     exit=$D_AB_CODE wall=$((D_T1 - D_T0))ms"
echo "     $(head -c 500 <<<"$D_AB_OUT")"
if [ "$D_AB_CODE" -ne 0 ]; then
  pass "[$STEP] 12.5 the unresolvable anchor was REFUSED (exit $D_AB_CODE)"
else
  fail "[$STEP] 12.5 an unresolvable anchor did not refuse (exit 0)"
fi
if [ "$D_IDX_BEFORE" = "$D_IDX_AFTER" ]; then
  pass "12.5 the index vector is byte-identical — nothing was posted"
else
  fail "12.5 the index vector MOVED: $D_IDX_BEFORE -> $D_IDX_AFTER"
fi

# --- 12.6  the SPARSE=0 A/B on the closed-loop drop -------------------------
echo ""
echo "--- 12.6  the A/B: the same move, sparse census vs the full sweep ---"
D_AB_BASE="$(area_order)"
D_AB_MOVER=$(first_area "$D_AB_BASE")
D_AB_U=$(db "SELECT uuid FROM TMArea WHERE title='$D_AB_MOVER' ORDER BY \"index\", uuid LIMIT 1")
sb_move "12-6a-ab-sparse" 0 "" -- area reorder "$D_AB_U" --last --dangerously-drive-gui --verify-timeout 240000
D_AB_SPARSE="$SB_AFTER"; D_AB_SPARSE_COST="$SB_COST"
sb_move "12-6b-ab-restore" 0 "" -- area reorder "$D_AB_U" --first --dangerously-drive-gui --verify-timeout 240000
if [ "$SB_AFTER" = "$D_AB_BASE" ]; then
  pass "12.6 the A/B baseline is restored"
else
  fail "12.6 could not restore the A/B baseline (got $SB_AFTER, wanted $D_AB_BASE)"
fi
sb_move "12-6c-ab-sweep" 0 "THINGS_API_SIDEBAR_SPARSE=0" -- area reorder "$D_AB_U" --last --dangerously-drive-gui --verify-timeout 240000
D_AB_SWEEP="$SB_AFTER"; D_AB_SWEEP_COST="$SB_COST"
if [ "$D_AB_SPARSE" = "$D_AB_SWEEP" ]; then
  pass "12.6 SPARSE=0 landed the IDENTICAL order: $D_AB_SWEEP"
else
  fail "12.6 the two censuses disagree — sparse=$D_AB_SPARSE sweep=$D_AB_SWEEP"
fi
D_SW_SPARSE=$(cost_field "$D_AB_SWEEP_COST" "sparse")
D_SW_ENABLED=$(cost_field "$D_AB_SWEEP_COST" "sparseEnabled")
if [ "$D_SW_ENABLED" = "False" ] && [ "${D_SW_SPARSE:-1}" -eq 0 ]; then
  pass "12.6 the SPARSE=0 arm really ran the full sweep (sparseEnabled=False, sparse censuses=0)"
else
  fail "12.6 the SPARSE=0 arm did not fall back (sparseEnabled=$D_SW_ENABLED, sparse censuses=$D_SW_SPARSE)"
fi
echo "     A/B cost, same move, one env var apart:"
echo "       sparse: $D_AB_SPARSE_COST"
echo "       sweep : $D_AB_SWEEP_COST"

# --- 12.7  a no-scroll-bar --last -------------------------------------------
echo ""
echo "--- 12.7  --last with the whole list inside the viewport (no scroll bar) ---"
# DRPLC1 §3 ruled OUT the scroll-bar hypothesis in the lab; this cell is the
# release-side re-run of that control, because the aim it exercises is the one
# that lies outside the collapsed content.
D_PW=$(db "SELECT COALESCE(group_concat(uuid,' '),'') FROM TMTask WHERE type=1 AND trashed=0 AND title LIKE '$RCTAG-DPW%'")
for U in $D_PW; do
  things project delete "$U" --json >/dev/null 2>&1
done
sleep 2
echo "     projects left: $(db 'SELECT count(*) FROM TMTask WHERE type=1 AND trashed=0')"
setwin 1200 900
echo "     Things standard window: $(getwin)"
D_M7=$(first_area "$(area_order)")
D_M7_U=$(db "SELECT uuid FROM TMArea WHERE title='$D_M7' ORDER BY \"index\", uuid LIMIT 1")
sb_move "12-7-fits-last" 0 "" -- area reorder "$D_M7_U" --last --dangerously-drive-gui --verify-timeout 240000
[ "$(last_area "$SB_AFTER")" = "$D_M7" ] \
  && pass "12.7 \"$D_M7\" landed last with no scroll bar" \
  || fail "12.7 the no-scroll-bar --last did not land — after: $SB_AFTER"
D_CHEV7=$(cost_field "$SB_COST" "g.chevron")
if [ "${D_CHEV7:-0}" -eq 0 ]; then
  pass "12.7 no fold was needed — the wall is gone and the list is not blocking"
else
  fail "12.7 the drive still folded something (${D_CHEV7} chevron gestures) — the list did not fit"
fi
echo "     scroll fraction at the drop (0 = the whole list is inside the viewport):"
printf '%s\n' "$SB_DROPS" | python3 -c "
import json, sys
for line in sys.stdin:
    line = line.strip()
    if not line: continue
    for it in (json.loads(line).get('iterations') or []):
        print('       scroll=%s rows=%s' % (it.get('scroll'), it.get('rows')))
"

# --- CELL 12 invariants ------------------------------------------------------
echo ""
echo "--- CELL 12 epilogue: the invariants ---"
D_END_AREAS=$(db 'SELECT count(*) FROM TMArea')
D_END_DIGEST=$(db "SELECT COALESCE(group_concat(u,','),'') FROM (SELECT uuid||':'||COALESCE(area,'') AS u FROM TMTask WHERE trashed=0 ORDER BY uuid)" | shasum | cut -c1-12)
if [ "$D_END_AREAS" = "$D_SEED_AREAS" ]; then
  pass "12.x the area COUNT is unchanged from the seed ($D_END_AREAS) — no area was minted or lost"
else
  fail "12.x the area count moved: seed=$D_SEED_AREAS now=$D_END_AREAS"
fi
echo "     assignment digest: seed=$D_SEED_DIGEST now=$D_END_DIGEST (12.7 deletes the wall's projects, so a change here is expected)"
echo "     collapsedAreaUUIDs: [$(collapsed_uuids | tr '\n' ' ')]"
echo "     final area order:   $(area_order)"
