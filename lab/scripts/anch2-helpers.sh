#!/bin/bash
# Install ANCH2 guest helpers into ~/labh (persistent — survives large clock
# jumps that trigger the /tmp cleaner). Does NOT touch clock/airgap. Idempotent.
set -uo pipefail
cd "$(dirname "$0")/../.."
source lab/scripts/env.sh
VM="anch2-lab"; IP=$(tart ip "$VM" 2>/dev/null || true); [ -n "$IP" ] || { echo "no IP"; exit 1; }
...
lab_ssh "$IP" 'cat > ~/labh/axsetdt.jxa' <<'EOF'
...
EOF
echo "helpers installed to ~/labh"
