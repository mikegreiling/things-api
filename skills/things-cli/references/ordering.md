...
- **Resolved to-do movees (LOGSORT):** a SWEPT completed/canceled to-do (a Logbook resident) is refused, pointing at `things todo reopen` (reactivation) or `--completed-at` (to re-date it in the Logbook — sort order can't express Logbook re-dating). An unswept resolved to-do gets a distinct reopen-first refusal.
