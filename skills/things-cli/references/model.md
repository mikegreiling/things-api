...
- **`type` is presence-keyed:** **absent `type` = to-do.** A row omits `type` when it is a to-do; `type` is present for `project`, `heading`, `area`, and `tag` (including in the error `candidates` shape).
