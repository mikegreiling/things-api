...
Namespaced verb families — run `things <group> --help` for the verbs and `things <group> <verb> --help` for exact flags: `things todo …` (add/update/complete/cancel/reopen/move/reorder/delete/restore/tags/checklist/make-repeating), `things project …` (add/update/move/complete/… plus the heading verbs `add-heading`/`rename-heading`/`archive-heading`/`unarchive-heading`/`promote-heading`/`move-heading`/`move-heading-to-project`, each taking a project ref then a heading selector — an exact title or uuid; `move-heading` reorders headings WITHIN a project, while `move-heading-to-project <src> <heading> --to <dst>` relocates one heading and its to-dos to a DIFFERENT project, a GUI-driven op needing `ui.enabled` + `--dangerously-drive-gui`), `things area …`, `things tag …`, plus `things batch` (JSONL of many changes — chain created uuids across lines with `tempId`/`$ref`, retry safely with `opId`, undo the whole run with its `undoToken`; see references/contracts.md), `things undo`, and `things reorder` (whole-list/container reorder by scope).
