...
Namespaced verb families — run `things <group> --help` for the verbs and `things <group> <verb> --help` for exact flags: `things todo …` (add/update/complete/cancel/reopen/move/reorder/delete/restore/tags/checklist/make-repeating), `things project …` (add/update/move/complete/… plus the heading verbs `add-heading`/`rename-heading`/`archive-heading`/`unarchive-heading`/`promote-heading`/`move-heading`/`move-heading-to-project`, each taking a project ref then a heading selector — an exact title or uuid; `move-heading` reorders headings WITHIN a project, `move-heading-to-project <src> <heading> --to <dst>` relocates one heading and its to-dos to a DIFFERENT project, and `dissolve-heading <project> <heading>` removes a heading while KEEPING its to-dos as direct project children (NOT trashed — the opposite of the Shortcuts heading-delete cascade); the last two are GUI-driven ops needing `ui.enabled` + `--dangerously-drive-gui`), `things area …`, `things tag …`, plus `things batch` (JSONL of many changes — chain created uuids across lines with `tempId`/`$ref`, retry safely with `opId`, undo the whole run with its `undoToken`; see references/contracts.md), `things undo`, and `things reorder` (whole-list/container reorder by scope).

**Move vs reorder — keep them apart** (`things help move`). MOVE changes WHAT an item belongs to (membership somewhere): `things todo move <refs…> --to-project|--to-heading|--to-area <ref>`, or the detach family `--no-heading` (leave the heading, stay in the project) / `--loose` (leave heading, project, AND area) / `--inbox`; `things project move <refs…> --to-area <ref>|--no-area`. There is no `--detach` (removed) and no `--no-area` on a to-do (its area is inherited — use `--loose`). REORDER changes only ARRANGEMENT in place and never membership: `things todo reorder <refs…>` assembles the named to-dos as one block at the EARLIEST one's current slot in argument order (partial-selection friendly — unmentioned siblings keep their order); `--first`/`--last`/`--before <ref>`/`--after <ref>` position it. Both verbs are variadic (selection order = landing order; reverse by naming them backwards) and take those position flags; an anchor positions but never migrates. Placement "top of bucket" is GUARANTEED across loose lists (inbox/today/evening/someday/anytime), a project's or area's members (anytime AND someday), a heading's anytime children, and a container's same-day scheduled children; the result's placement class says which you got, and a bounce that co-bounces unnamed siblings to honor a `--before`/`--after` anchor lists them. Two config knobs tune the when= bounce protocols: `bounce-max-items` (default 30, caps items a bounce touches) and `bounce-enabled` (default on; off refuses bounce-dependent placements rather than degrading destructively). The private native reorder command is on by default (`allow-experimental`, the off-switch).

**Scheduling is an update, not a move**: `things todo update <ref> --when today|evening|anytime|someday|YYYY-MM-DD` schedules or parks an item; `move` changes its CONTAINER only. Per-verb preconditions and the rest of the write vocabulary: `things help writes`, `things help move`, `things help repeating`, [references/contracts.md](references/contracts.md).
