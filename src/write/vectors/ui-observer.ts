 ...
 *
 * ── ONE HOST CLASS CANNOT HAVE IT: the deputy-routed Mac (#695) ────────────
 *
 ...
 * the spawn hop, not the in-script client. 0.20.7 shipped the sidecar without
 * that gate and `todo add-repeating --dangerously-drive-gui` failed in two
 * seconds on every helpers-routed Mac — the lab never saw it because goldens run
 * scripts DIRECT, with no deputy installed, so the certified arm was the only arm.
 *
 * So deputy routing is simply one more reason {@link observerAvailable} says no,
 * and the fallback below does the rest. The observer stays live on every
 * direct-execution host. The proper long-term home for an AXObserver is the
 * trusted long-lived process itself — a deputy-hosted observer, or a deputy verb
 * that spawns the sidecar under the helper's identity — which is a deputy
 * version bump plus a helpers reinstall, and is the maintainer's call
 * (docs/lab/vopat2-ax-settle-observer.md §routed arm).
 *
...
import { deputyExpected } from "../../deputy/routing.ts";
...
let availability: { ok: boolean; why: string } | null = null;
...
/**
 * Can a sidecar run on this Mac? Gated exactly as VOPAT1's rig gated it: the
 * Command Line Tools are what put `python3` on the PATH, and `xcode-select -p`
 * is the prompt-free question that says whether they are installed. Memoized —
 * the answer cannot change inside one process.
 *
 * ONE GATE COMES FIRST, AND IT IS NOT ABOUT THIS MACHINE'S TOOLS (#695): a host
 * that expects the deputy to carry its automation cannot have the sidecar,
 * because both of the sidecar's hops shell out and the broker's lint refuses
 * every script that does. EXPECTED, not merely active, is the right question:
 * when the deputy is expected but not carrying traffic the osascript seam
 * REFUSES rather than running direct (#620, src/deputy/osa.ts), so there is no
 * configuration where an expected deputy still executes an acting script
 * locally. The check is prompt-free and socket-free — config plus a file
 * existence test — so asking it costs nothing and can never raise a dialog.
 * It is deliberately OUTSIDE the memo: routing is a per-process decision of its
 * own, and the test seams reset the two independently.
 */
export async function observerAvailable(
  env: NodeJS.ProcessEnv = process.env,
): Promise<{ ok: boolean; why: string }> {
  if (observerDisabled(env)) return { ok: false, why: `switched off by ${OBSERVER_ENV}` };
  if (deputyExpected(env)) {
    return { ok: false, why: "deputy-routed: brokered scripts cannot spawn the sidecar" };
  }
  if (availability !== null) return availability;
  if (!(await which("/usr/bin/xcode-select", ["-p"]))) {
    availability = { ok: false, why: "the Command Line Tools are not installed (xcode-select -p)" };
    return availability;
  }
  if (!(await which("/usr/bin/python3", ["-c", "import ctypes,socketserver"]))) {
    availability = { ok: false, why: "/usr/bin/python3 cannot load ctypes" };
    return availability;
  }
...
function request(session: ObserverSession, line: string, timeoutMs: number): Promise<Reply | null> {
  return new Promise((resolve) => {
    let settled = false;
    let buf = "";
...
export async function startObserver(
  ...
  const avail = await observerAvailable(env);
  if (!avail.ok) {
    trace(() => ({ phase: "ui-observer", event: "unavailable", why: avail.why }));
    return null;
  }
  ...
  const session: ObserverSession = { socketPath, token, logPath, registered: "?", pid: 0 };
...
/**
 * Ask the sidecar to exit, and say honestly whether it answered. Called from the
 * drive's `finally`; the sidecar's TTL and idle timeout are what cover the case
...
export async function stopObserver(session: ObserverSession): Promise<boolean> {
  ...
  trace(() => ({
    phase: "ui-observer",
    event: "stopped",
    acknowledged: reply !== null && reply.ok,
    sidecarPid: session.pid,
  }));
