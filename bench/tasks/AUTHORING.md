...
### Simulator WRITE-coverage gap — CLOSED (0.14.0 write round, #430)

~~The bench simulator has no `reorder` applier and does not apply `--created-at`/`--completed-at`.~~ **LANDED in #430** (`src/write/vectors/simulator.ts`), RSIM-style / law-grounded: universal `reorder` (ORD-1 native-wire re-rank on the correct bucket axis, ORD-18 axis-isolation, LOGSORT ORD-13 byte-lock) and resolution timestamps (`todo/project.set-dates` exact stamps, WG-7 flip-preserves-stopDate, born-timestamped `add`). So universal reorder across kinds AND backdating flows are now gradable — see the write tasks below. **Still out (unsupported beats guessed):** the BOUNCE-only reorder scopes (evening / anytime / projects / heading / area-someday / day — their front/back-insert index mechanics are unproven under the sim), and `cancel --completed-at` / `update --completed-at` on a canceled PROJECT (no `project.cancel` applier). So a reorder task must use a NATIVE scope (today / project / area / inbox / someday / container-day / tomorrow), and a project backdating task must use `complete --completed-at` (not cancel).

## Write-round tasks (0.14.0, 2026-08-06)

Five write tasks exercising the newly-gradable surface (corpus 36→41). Encoding facts:

- **Reorder assertion pattern:** SQL grading is order-sensitive deep-equal, so assert the resulting order directly — `SELECT title FROM … WHERE title IN (…) ORDER BY todayIndex` (Today/day scopes) or `ORDER BY t."index"` (project/area/inbox/someday) and expect the row objects in target order. No UUID literals (seeds re-roll); join through titles as usual. The applier re-ranks the scope's members among their own rank slots (permuted into the sent wire order), so a partial request keeps the unrequested tail below.
- **`--in` token:** `things reorder <refs…> --in <target>` — `--in today` / `--in evening` / `--in someday` / `--in inbox` are stage-list axes; `--in <project/area title>` is a container axis; a YYYY-MM-DD or `upcoming` is a day axis. Only today / evening / a day-block / upcoming intermix to-dos AND projects; a container or stage-list axis sorts ONE kind at a time (the guard refuses a mixed set — the basis of `writes-reorder-refuse`).
- **Backdate stamps are Unix-epoch seconds** (NOT packed dates): `stopDate` / `creationDate` = `Math.floor(resolveResolutionInstant(iso, zone).getTime()/1000)`, a date-only value landing at NOON in the effective zone (America/Chicago here). Verified literals for the pinned clock: `2026-07-10` → `1783702800`; `2026-07-05` → `1783270800`. Assert `status=3 AND stopDate=<epoch>` for a completed backdate.
- **Refusal-correctness task** (`writes-reorder-refuse`): a mixed-kind area reorder is refused by the H-REORDER-SCOPE guard; grade with `db-unchanged` (the machine-checkable refusal signal — a mutating "workaround" fails) PLUS an `answer.possible=false`. NOTE: the `possible` field is dropped by ~40% of agents (both arms) who narrate the refusal in prose — `db-unchanged` still held in every observed run, so refusal comprehension was 100%; the answer-shape is the fragile part (cf. the `gui-evening-tonight` bare-json artifact). Candidate: reduce to `db-unchanged`-only.

Task inventory (id · split): `writes-reorder-today-intermix` · dev — cross-kind Today reorder (`--in today`, to-dos + a project on `todayIndex`); `writes-reorder-project` · dev — a project's unheaded children (`--in <project>`, `index`); `writes-reorder-refuse` · validation — mixed-kind area reorder → refused; `writes-backdate-complete` · dev — `complete --completed-at`; `writes-add-logged` · validation — born-resolved `add --completed-at`.
