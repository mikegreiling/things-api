...
<!-- ledger-entry id="rename-round-data-model" lesson="Aligning a reference file's NAME to the concept agents reach for beats negating the wrong guess: renaming references/model.md → data-model.md took the pi skill arm's failed data-model.md opens from 5/12 runs to 0, where the prior round's inventory negation had left the guess persisting (18 mentions). The friction only exists under STATIC skill injection — the native-ingestion (Claude Code) arm never hunts reference filenames, so the guess-prior does not manifest there at all." -->
### 2026-08-07 · skill · rename-round (data-model) · **SHIPPED**

- **what:** the deferred "data-model mis-guess suppression" candidate — rename `references/model.md` → `data-model.md` (+ SKILL.md inline links, inventory `model`→`data-model`, and drop `data-model` from the "there is no … file" negation). Content byte-identical; only the name + pointers change. First cross-family paired round: measured within BOTH arm families.
- **protocol:** paired A/B via `BENCH_SKILL_DIR` (baseline vs rename skill dirs in scratch; in-tree skill untouched until ship). 4 tasks (`reads-project-heading-recursion` dev, `reasoning-effective-tags` dev, `reads-upcoming-dayblock` validation, `reads-today-counts-holdout` holdout) × 3 reps × {baseline, rename} × {pi/gpt-5.4-mini via openai-codex, claude-skill/Haiku} = 48 runs, world-seed 1. Wrong-guess metric = bash COMMANDS (pi `toolCall.arguments.command` + claude `tool_use.input.command`) naming the nonexistent reference file per config (baseline: data-model.md; rename: bare model.md) — commands only, so advert/skill-content echoes don't pollute the count.
- **measured (success · safety · wrong-guess runs · friction mean-err):**
  - pi-baseline 10/12 · ok · **5** (failed data-model.md opens) · 0.40
  - pi-rename 11/12 · ok · **0** · 0.55
  - claude-baseline 12/12 · ok · 0 (0 reference-file reads at all) · 0.33
  - claude-rename 12/12 · ok · 0 (0 reference-file reads) · 0.33
- **marquee verdicts:** (a) pi guess-friction ELIMINATED (5→0; the constant `SKILL_ADVERT` names `data-model`, so the rename makes those references resolve instead of fail); (b) lineage — Haiku does NOT exhibit the `data-model.md` prior (native ingestion never reaches for a reference filename); (c) native ingestion sidesteps the reference-hunt friction that static injection provokes.
- **decision (constitution ladder):** safety clean all cells · success non-inferior-or-better (pi 10→11/12, claude 12→12/12) · targeted friction eliminated for pi + neutral for claude → **SHIP.** Directional at N=3 (both subscriptions quota-bound) — flagged; the effect is a mechanical filename match (not a noisy success delta), and the prior ≥10-rep confirmation already documented the persistent guess this resolves.
- **open (deferred):** the pi `SKILL_ADVERT` (`bench/prompts/system.ts`) still enumerates non-existent `reads`/`writes`/`recurrence`/`safety-and-recovery`; aligning it to the real file set would further cut pi reference-hunt friction (it was constant across this A/B, so it did not confound the result).
- **artifacts:** bench/artifacts/ — pi-baseline/pi-rename/claude-baseline/claude-rename (this round's raw runs live in the run scratch; not committed).

<!-- ledger-entry id="advert-postmortem-stale-skill-advert" lesson="Reference-hunt friction charged to the skill arm across #428/#432/#435 was seeded by a STALE SYSTEM PROMPT, not a model prior: the bench's SKILL_ADVERT enumerated Phase-0 reference filenames that two skill restructures had renamed, and every 'guessed' filename was on that stale list. When copy is duplicated across a surface AND its advert, the advert rots silently and gets misread as model behavior — enumerate once (in the surface's own inventory) and route to it, never duplicate the list in the fixed prompt." -->
### 2026-08-09 · skill · advert post-mortem (stale SKILL_ADVERT) · **SHIPPED (structural fix, no measurement round)**

- **what:** root-cause discovery, not a refinement candidate. `bench/prompts/system.ts` `SKILL_ADVERT` (the pi skill arm's static-injection system prompt) enumerated the Phase-0 reference set `(reads, writes, gui, data-model, recurrence, safety-and-recovery)` — authored correctly at #196 (2026-07-17; verified via `git ls-tree f334e7c skills/things-cli/references/`) but never swept when #246/#359 renamed the set to `data-model`/`contracts`/`ordering`/`errors`/`banner`/`gui`.
- **re-attribution (the finding):** every "guessed" reference filename charged against the skill arm across #428 (`data-model.md`, `reads.md`, `writes.md`, `recurrence.md`, `safety-and-recovery.md`), #432, and #435 is *exactly* an entry on this stale advert list. The friction was advert-seeded (an in-context instruction to read files that don't exist), not a model prior. Consequently: (a) #428's "reference sprawl is a NET friction ADD" — the additive SKILL.md inventory paragraph was fighting an in-context CONTRADICTION (advert vs SKILL.md's own inventory), not intrinsic sprawl; (b) #435's rename flipping the `data-model.md` guess 5/12→0 — the rename made ONE stale advert claim true (`data-model.md` now resolves) while the other four kept seeding failed opens; (c) the persistent `data-model.md` "model prior" of #428/#432 — primarily advert-seeded, same class as the rest.
- **fix (non-enumerating, chosen over refreshing the list):** dropped the advert's filename enumeration entirely; it now routes the agent to SKILL.md's own authoritative, self-negating line-13 inventory. Less rot-prone than a corrected list: the enumeration was a duplicate of SKILL.md's inventory and the duplicate is what rotted — removing it lets the reference set change with no sweep of `system.ts`. Guard comment added in `system.ts` recording that this rot class is now structurally closed. `PROMPT_VERSION` v0→v1; the skill/claude-skill `promptHash` changes → **re-baseline flag** on any cross-round skill-arm comparison spanning this commit (the bare-`cli` prompt is unchanged).
- **why no measurement round:** the constitution's ladder/promotion rules govern the three consumer surfaces UNDER TEST, not the bench's own fixed system-prompt scaffolding; nothing in it mandates a paired round or maintainer sign-off for a harness-prompt correction (only CONSTITUTION.md edits need Mike's sign-off). This is a factual bug fix (the advert named files that do not exist), not copy tuning under promotion; the advert was a CONSTANT across the #428/#435 A/Bs, so it never confounded those deltas — only their attribution, corrected here. Directionally the change can only reduce pi reference-hunt friction (it removes the seed of the failed opens).
- **debrief:** attribution — mechanical (a filename mismatch between a fixed prompt and the mounted surface); confidence — high (every documented guess maps 1:1 onto an advert entry). lesson — see marker.
- **artifacts:** none (discovery + structural fix; the evidence is the prior rounds' transcripts already in bench/artifacts/v2round*/ and rd-*/).
