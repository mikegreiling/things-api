...
**Build status (2026-07-15):** `area.reorder-sidebar` SHIPPED + lab-certified (AXDRAG2, PR pending merge) — sidebar area order (P6/O13) is closed. **Parked from that build:** (1) the scroll-while-held ladder rung is built + probe-certified but ships disabled (`THINGS_UI_DRAG_LADDER=held-scroll` lab knob) until the sidebar AX-mirror ghost bug (oddities §9) is understood or Cultured Code fixes it; (2) **`tag.reorder` is blocked-on-judgment** — nameless positional rows + ubiquitous `TMTag."index"` ties make aiming unverifiable (docs/lab/axdrag2-reorder-certification.md §AXDRAG2-d); revisit only with a name handle or a validated tie-break oracle for the Tags-window order.
