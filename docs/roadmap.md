...
Lab findings so far (SX4): stock VMs can't self-grant Accessibility (TCC.db is SIP-protected read-only even with FDA — "attempt to write a readonly database"), so in-VM AX is blocked. **The lab arm is VNC synthetic input**: `tart run --vnc-experimental` exposes a standard RFB server; synthetic mouse/keyboard events arrive as hardware-level input, bypassing TCC entirely. **First working demonstration 2026-07-10 (SX6)**: vncdotool (host venv) clicked a Shortcuts import sheet's "Add Shortcut" button in a clone — capture the framebuffer (`vncdo capture`, coordinate-true), read the button position off it, `vncdo move X Y click 1`; the sheet layout is deterministic across clones, so a two-phase capture-then-click script works headlessly (`lab/scripts/research-sx6.sh` is the template). That same arm would automate consent-prompt clicks (even the per-run delete-class prompts) in probes. Feasibility probe queued: drive `File → New Repeating To-Do` end-to-end via VNC clicks in a clone, verify an `rt1_recurrenceRule` row lands. Alternative: a SIP-disabled derived VM image (boot recovery via `tart run --recovery`, `csrutil disable`) makes TCC.db writable → real AX scripting in-VM.
...
**Width-handling polish pass (parked by Mike, 2026-07-12 — "nice-to-have future pass").** Long titles currently soft-wrap wherever the terminal breaks them, leaving trailing tags/chips unindented. Two candidate treatments to weigh at pick-up time, TTY-only via `process.stdout.columns` with a ~60-col minimum clamp:
1. **GUI-parity single-line truncation (Mike's leaning)**: titles overflow with `…`, tag chips collapse to `…` — deterministic, never goes stale on resize, matches both the Things GUI and the `docker ps` column idiom.
2. Hanging-indent hard wrap (brew/cargo idiom): reads better for long titles but any printed wrap goes stale the moment the window is resized (soft-wrap stacks on hard-wrap); universally accepted wart, but strictly worse than (1) on that axis.
Full titles remain one `things show <id>` away either way, which weakens the case for (2).
