...
## Agent daemon — β1 permission deputy SHIPPED; remaining phases awaiting ratification

One resident signed launchd helper solving three converged problems: agent-harness timeout caps (async op contract + `accepted` results), TCC grant churn, and `things watch` residency. **β1 — the permission deputy (`things-deputy`) — shipped 2026-08-19** at the maintainer's direction: a dumb privileged broker (read-only SQL, fixed-shape osascript, container file reads) that becomes the stable TCC grantee; opt-in (`deputy-enabled`), direct fallback, live-certified on CI macOS ([design/agent-daemon.md](design/agent-daemon.md) §3a; real-TCC validation gate open pending EDR exclusion or the M1 — see up-next). Remaining phases α (watch engine in-process) → β (async contract + spool, rides the deputy) → γ (watch hosted) → δ: **[design/agent-daemon.md](design/agent-daemon.md)**. Next step: maintainer ratifies that plan, then phase α.
...
## Someday / maybe — parked by the maintainer (2026-09-07)

Items he has looked at and set down. Each is revived by a named trigger, not by a review pass.

- **Stateful display preferences — a config surface for the TTY toggle family.** The `--show-later`/`--show-logged`/`--until`/`--all` toggles and the limit knobs (`--limit`/`--area-limit`/`--project-limit`) want user-configurable defaults, plus three deferred knobs: `limitMode fixed|fit|none` (height-fit deliberately NOT the default — determinism beats cleverness), `pager auto|never` (git-style `less -FRX`), and a `--plain`/`--pretty` chrome override pair (`--plain` = the flag form of `NO_COLOR=1 THINGS_WIDTH=0`; `--json` ignores all of it so agents stay unconfused). A config surface already exists to hang it on (`src/config.ts`, `things config set`), carrying policy keys only; the open question is precedence, and the standing wariness is defaults that change output without a flag changing — which is what confuses an agent. Mike, 2026-09-07: "can go on a someday, maybe list. idk, if its important". Trigger: he asks for the defaults, or a real workflow keeps retyping the same flags.
- **The free-Apple-ID amfid cell.** Whether a FREE Apple Development certificate (any Apple ID, no paid membership) satisfies amfid for the sandboxed reader — the self-builder path's load-bearing assumption, asserted from cert-class reasoning and never measured. It cannot be automated: minting one needs an interactive Apple ID sign-in (Xcode ▸ Settings ▸ Accounts) inside the VM, and no credential may enter this repo. Trigger: the first external build report — a stranger actually attempting the self-builder path.
- **The behavioral canary for the private reorder command.** With heading order restored on chords (#606/#608) and day-groups movable on them (ORD-19), the inert `_private_experimental_ reorder` command carries little of the ordering load, so the layered attempt-and-memoize design stays a DRAFT ([design/reorder-canary.md](design/reorder-canary.md), with its three ratification points); the declaration check and the measured-build table stay as they are, and the o-suite's `deltaEmpty` rows remain the lab-side canary. Trigger: a future Things build reactivating the command.
