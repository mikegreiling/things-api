...
Still open for §A:
1. ~~Import validation on real hardware~~ — **DONE 2026-07-10**: signed-file import+run validated on Mike's Mac; iCloud-free distribution works. BUT the retest exposed that `things-proxy-find-items` was mis-built (echoes input, doesn't search — S01 confound corrected; product-impact NONE, it's diagnostic-only). **IN-FLIGHT VM WORK (this thread):** repair the find-items filter serialization in a golden clone + discriminate the malformed-predicate crash (see s-campaign-results.md "Real-hardware validation + corrections (2026-07-10)" — that section IS the durable task list). Key lead: the header note says the name field is `Title`, so the filter Property is likely `title` (I crashed prod with `name`). A working hand-authored filter would unlock **programmatic proxy authoring** (compose in Python, no golden GUI sitting) — a novel path that changes §A.2 below.
