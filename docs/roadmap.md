...
- **Container/list extension — SHIPPED 2026-07-16 (`mg/overdue-containers`).** `--overdue` now also scopes `things projects`, `project show`, and `area show` (plus their `things <id>` sugar) with **OWN-DEADLINE UNIFORM** semantics (Mike ruling 2026-07-16): keep each listed entity iff its OWN deadline is overdue, no recursion. `things projects` filters project rows (projects carry a `deadline`); `project show` filters child to-dos and collapses emptied headings; `area show` filters loose to-dos AND child projects by each row's own deadline with no descent into project contents. The container "no strict `--limit`" doctrine is preserved (it narrows content, never lifts/imposes limits). `things areas` (the area LIST) rejects it fail-closed (areas are not dated); `things areas <ref>` accepts it as an `area show` synonym. MCP parity on `get_project`/`get_area`/`list_collections`. Safe here despite the containers not accepting `--tag` because deadlines are NOT inherited (see the parked item below).

Status: **shipped.**

**Parked follow-up — `--tag` (and other content scopes) in the container views.** The container/list views (`project show`, `area show`, `things projects`) do NOT yet accept `--tag`/`--untagged`. `--overdue` was safe to add first because a deadline is an item's OWN column — never inherited — so "keep entities whose own deadline is overdue" is unambiguous. `--tag` is NOT: the read model treats an item as carrying its project's and area's tags (inherited-tag semantics, oddities **§9a**, GUI-verified TAGINH2). Filtering a project's children by a tag the PROJECT itself carries would match every child (the tag is inherited by all of them) — vacuous — and "No Tag" inside a container is likewise skewed. That needs its own ruling on whether container `--tag` means own-tag-only, inherited-inclusive, or is refused. Deferred until then; do not wire `--tag`/`--untagged` into the containers without settling §9a first.
