...
2. **Wire the Shortcuts write vector** into the pipeline (`heading.create`, dated-reminder clear): `VectorId "shortcuts"`, matrix entries from S-campaign evidence, `shortcuts run` executor, remediation strings pointing at `things setup shortcuts`. NEW since 2026-07-10: proxies for these can be authored programmatically (see §A.1) instead of waiting on a golden sitting if the golden-resident set ever needs to change. **EXECUTION PLAN (2026-07-11): [design/shortcuts-write-vector-plan.md](design/shortcuts-write-vector-plan.md)** — written for a delegated agent (branch `mg/shortcuts-write-vector`); the executing agent must never `shortcuts run` a mutating proxy on the host.
