...
**MCPSRV1 — Things' own MCP server (not an AI324 item; its own campaign). PHASE 1 IS ANSWERED — see [lab/mcpsrv1-static-recon.md](../lab/mcpsrv1-static-recon.md).** The hypothesis held and then some. Things carries a **complete** in-process MCP server: protocol `2025-06-18`, newline-delimited JSON-RPC 2.0 over a unix socket `mcp.sock` in the app group container `JLMPQHK86H.com.culturedcode.ThingsMac`, the generic stack in Cultured Code's own `FoundationAdditions.framework` and the Things half in `ThingsCommon`. It advertises exactly one capability (`tools`) and exposes exactly **one tool, `GetTodayList`** — a paginated ThingsJSON **read**, `{cursor?: string}` in, `{items, nextCursor?}` out. No write tools, no resources, no prompts, no peer-credential check. It is off behind the internal flag `THCFeature.mcpServer`, whose activation is an `NSUserDefaults` boolean keyed by an obfuscated UUID and for which no settings UI ships; it first appeared in **3.23** and is unchanged in substance through 3.24; and it is **not running on the maintainer's host**. **PHASE 2 IS ALSO ANSWERED, and the answer is negative — see [lab/mcpsrv1-guest-probe.md](../lab/mcpsrv1-guest-probe.md)** (2026-09-14, golden-v5 / Things 3.24 build 32400006 / macOS 15.7.7 / dbv 29). The flag is **necessary but not sufficient**. The activation key derives correctly — verified twice, including a disassembly of `FADeobfuscatedUUID` itself — and lands in the app's *real* preferences home, the app-group container plist, where it survives a quit and relaunch alongside `uriSchemeEnabled`. Things then launches, opens its database, builds its windows, and holds **zero unix-domain sockets**: no `mcp.sock`, under four candidate defaults domains, under all four at once, as a boolean or an integer, backgrounded or fully activated with a sixty-second settle. **Two other `.explicitThroughUserDefaults` flags with visible oracles (`copySelectionAsJSON`, `experimentalNewMenus`) do not flip either**, so the gate belongs to the feature-toggle machinery on a stock build, not to `mcpServer`. Reading further: `update()` gates solely on `isEnabled`; `isEnabled` has exactly one writer, in `Things3`, computing `features.contains(.mcpServer)` from a **captured** array that looks like `didChangeFeaturesEnabledNotification`'s payload; and `isFeatureEnabled` does reach the defaults read and would answer correctly. The likely break (MEDIUM confidence) is that **nothing assigns `isEnabled` at startup** — the feature can only be turned on from the in-app Feature Toggles panel, which is gated on a whitelisted Cultured Code account or a development build. Cells D–H never ran; the campaign wrote **zero** database mutations (3433 row-cells identical). *Opens:* nothing reachable. The ThingsJSON encoding-oracle value stands as a reason to want it, not as something we have. **No write vector is in prospect**, and no read vector either until the real gate is found.
