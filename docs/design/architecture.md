...
**Concurrency: the mutation lock** (`src/write/lock.ts`). An advisory lockfile (`<state dir>/mutate.lock`) serializes mutations across concurrent invocations — this process, another `things` CLI, the MCP server, the deputy's client. It protects create-probe verification from ambiguity, and every read-modify-write verb from the interleave that makes its mid-flight state a lie.

- **Two scopes.** PER LEG is the base case: `runMutation` takes the lock around one mutation. A COMPOSITE — one verb the engine runs as several mutations — instead holds ONE lock end to end, via `withMutationLock` (and its result-shaping wrappers `runComposite` for `MutationResult` verbs, `runLockedComposite` for the move/reorder verbs' own vocabularies). Reentrancy is carried on the ASYNC CONTEXT (`AsyncLocalStorage`), so a leg reached from inside the hold is a no-op while a genuinely parallel composite in the same process — two MCP tool calls — is excluded exactly as a second process would be (ruling 2026-08-23).
- **Who holds it end to end.** The promote verbs (`make-repeating`, `add-repeating`, the template-direct clone), the resolution-timestamp composites, and — since the 2026-09-05 ruling — every remaining multi-leg orchestrator: `todo.move` / `project.move`, the in-place and universal reorders, `reorder`'s bounce and scratch-container protocols, `area.reorder`'s sidebar drag, the whole-checklist edit, `clear-reminder`'s read-clear-restore, and the heading archive/unarchive pair. `area.reorder` is the sharpest case and the ruling's own example: it plans a drag from a sidebar census, so a concurrent write that adds, removes or re-ranks a row invalidates the plan mid-gesture.
- **What a contended writer gets.** It WAITS (30s by default; `WriteDeps.lockWaitMs`), then is refused — `blocked` / `lock`, in whatever refusal shape its verb speaks — with a sentence naming the holder: *"another operation holds the mutation lock: `area.reorder` (pid 4321), since 2026-09-05T10:02:11Z"*, plus how long it has been held, whether that process is still running, and (past five minutes, live holder only) that killing it releases the lock. A DRY RUN never takes the lock at all.
- **Every exit path releases.** The `finally` of whoever took it; an `exit` hook for the paths that never reach a `finally` (the CLI's interrupt guard answers SIGTERM/SIGINT with `process.exit`); and, for the uncatchable ones (SIGKILL, a power cut), the next acquisition's STEAL. A release only removes a file that still carries its own payload, so a mistaken or late release can never unlock a writer that has since taken it.
- **Staleness is an identity question, not a pid question.** The payload carries pid + process start time (`ps -o lstart=`), so a recycled pid cannot keep a dead writer's lock alive forever; ambiguity resolves conservatively to "alive" (a false-dead invites a double write, a false-alive only delays). `things rescue status` reports the holder without touching it.
...
**Undo selection (actor-scoping).** `things undo` replays inverse mutations off this trail. Three selection layers sit on top of the reversibility machinery: (1) **`--by <actor>`** (MCP `by`) filters targets to an exact recorded actor (or `*` for all) — `--by mcp` never matches an `undo:mcp` record, since inverse mutations are their own actor and are never themselves undo targets; (2) **asymmetric defaults** — the MCP undo tool defaults `by` to the connecting client's own derived actor (`mcp:<client>`, see MCP write policy below), so each session undoes only its own writes; an agent must pass `by: "*"` to touch the human owner's edits or another client's, while the CLI stays GLOBAL (the owner's Cmd+Z), with `--by <actor>` available to clean up after a specific agent; (3) **undo tokens** — every mutation result carries an `undoToken` (a compound op's shared txn id, or, for a single op, a content-addressed id derived from the record's `ts+op+actor+host+uuid`; see `undoToken` in `audit/schema.ts`), and `things undo --txn <token>` (MCP `txn`) inverts exactly that record, immune to interleaving. `--txn` is mutually exclusive with `--last`/`--by`. Inverse mutations carry an additive `undoOf: <token>` back-reference so an already-undone `--txn` request fails loudly rather than re-running.

**MCP write policy.** The MCP surface applies three deliberate restrictions on top of the shared write pipeline, because a remote MCP caller is assumed untrusted with the host's workspace. (1) **Actor identity is derived, not caller-settable.** The audit author for every MCP write comes from the client's own initialize-handshake identity — `clientInfo.name`, lowercased with non-alphanumerics collapsed to `-` and capped at 32 chars, prefixed to `mcp:<client>` (bare `mcp` when clientInfo is absent). No tool argument overrides it, so a caller cannot attribute its writes to another author, and the undo tool's default `by` scopes to this same string. (2) **The disruption ceiling is fixed at daemon startup.** `things mcp --allow-disruptive` / `--allow-very-disruptive` set `maxDisruption` (2 / 3) once for the whole process — the same mapping the CLI's per-call flags use — threaded into every write's `WriteOptions` (and each batch op). There is no per-request escalation; changing the ceiling means restarting the daemon. (3) **No config tool, no vector override.** MCP deliberately exposes neither a tool to mutate host config (profile, `ui-enabled`, `allow-experimental`) nor a per-call vector force. Changing write policy is a host-side action — restart with different flags, or edit the config file — not something the model can do over the wire. MCP tool arguments are snake_case throughout (the CLI keeps its kebab-case flags; internal `WriteOptions`/`BatchOp` fields stay camelCase).
