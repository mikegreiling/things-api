...
`repeating` follows the R11 template/instance split (reshape (5) above): the wire has NO `isTemplate`/`isInstance` discriminators — key presence carries the fact. A repeating TEMPLATE carries `repeating: {nextOccurrence, paused?, deadlined?, rule?, latestInstance?}` — the complete series object (`rule` and `latestInstance` on a detail read only; `nextOccurrence` an explicit `null` when unprojected; false booleans default-pruned). An INSTANCE carries a flat `instanceOf: <templateUuid>` and no `repeating`. A plain row carries neither. Absence of `repeating` therefore means "not a template"; absence of `instanceOf` means "not an instance". A template detail nests the `latestInstance` uuid inside `repeating` — the backward pointer symmetric to `nextOccurrence` (SL1).
