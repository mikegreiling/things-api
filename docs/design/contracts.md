...
- **Deadline pulls re-file to stage `anytime`; the `today` sections DROP `stage`.** A due/overdue unsuppressed deadline pulls an undated Inbox/Someday row OUT of its origin list and INTO Today + Anytime (BANNER1b law L-A) — `deriveStage` gains a step that derives the DESTINATION `anytime` for a Today-marked undated row, so EVERY Today member derives `anytime` and the today view's own sections become stage-PURE → `stage` is DROPPED there (was kept as "mixed" pre-R13). The flat `someday`/`inbox` views EXCLUDE pulled rows and the `anytime` view INCLUDES them via one shared predicate `DEADLINE_PULLED` (`src/read/predicates.ts`), reused verbatim from `todayView` so the four memberships can never drift; a SUPPRESSED deadline is not a pull (stays in origin, out of anytime/today).

**R10 dropping (bucket/section-implied).** `stage` is dropped where the enclosing view/section PROVABLY states it — the stage-PURE flat catalogues (`inbox`, `logbook`, `trash`, and the `anytime`/`someday` sidebar sections), the `today` view's own sections (R13 — every Today member derives `anytime`), and the stage-named card sub-buckets (`anytime`/`upcoming`/`someday`/`logbook`/`trash`) — and KEPT on the stage-MIXED / derived surfaces. The `anytime` catalogue is stage-PURE: every Anytime-view member (ANYTIME_SELF — undated-active, arrived-active, arrived someday-scheduled — plus, since R13, the deadline-PULLED undated rows the view now includes) derives `anytime`, because an ARRIVED `startDate` is Anytime not Upcoming (Upcoming is strictly future) and a pull re-files to Anytime, and templates are excluded from the view. `stage` is KEPT on `upcoming` (stage-MIXED — it also carries deadline-forecast `anytime`/`someday` rows), plus `search`, `changes`, the projects/areas listings, the card NODE, and detail. The derived `when` (R12) is dropped inside the `today` view's own sections (the section key states today/evening) and inside a card `upcoming` date-group keyed on it; kept everywhere else it is present (a logbook/trash row is never a Today member and no longer scheduled-forward, so it carries no `when` at all).
