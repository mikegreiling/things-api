...
- **Structural scaffolding** — the view shape that *carries* entities is not itself an entity and is never pruned, so its lists/records survive empty: the bucket RECORDS (`{items, total?}`, empty `items` kept) of the `today` view's two `children` buckets and the project-view's per-container `children` (`anytime`/`someday`/`logbook`, plus the `upcoming` day-block array), the project-view `headings[]` entries and their recursive `children`, the area card sections, and a sidebar section's `area: null` — the load-bearing "top-level / loose block" marker. Only the entities *inside* the scaffolding are pruned. (This is why omit-empty is scoped to recognized entity shapes, not a blanket deep prune: a to-do's `area: null` means "no area" and is dropped, but a section's `area: null` is a discriminant and is kept — same key, opposite meaning.)
