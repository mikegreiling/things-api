...
**Flat / chronological views** (`inbox`, `upcoming`, `logbook`, `trash`, `search`, `changes`) carry the base shape:
...
The `today` view carries the SAME base shape for its whole-view `shown`/`total`/`limit`/`truncated` (its `--limit` cuts across Today then This Evening in render order), but its per-bucket completeness lives INLINE on each `children` bucket record, not as a truncation sidecar (read-shape v2 R1) — see below.
...
- **Deadline pulls re-file to stage `anytime`; the `today` buckets DROP `stage`.** A due/overdue unsuppressed deadline pulls an undated Inbox/Someday row OUT of its origin list and INTO Today + Anytime (BANNER1b law L-A) — `deriveStage` gains a step that derives the DESTINATION `anytime` for a Today-marked undated row, so EVERY Today member derives `anytime` and the today view's two `children` buckets become stage-PURE → `stage` is DROPPED there (was kept as "mixed" pre-R13). The flat `someday`/`inbox` views EXCLUDE pulled rows and the `anytime` view INCLUDES them via one shared predicate `DEADLINE_PULLED` (`src/read/predicates.ts`), reused verbatim from `todayView` so the four memberships can never drift; a SUPPRESSED deadline is not a pull (stays in origin, out of anytime/today).

**R10 dropping (bucket/section-implied).** `stage` is dropped where the enclosing view/section PROVABLY states it — the stage-PURE flat catalogues (`inbox`, `logbook`, `trash`, and the `anytime`/`someday` sidebar sections), the `today` view's two `children` buckets (R13 — every Today member derives `anytime`), and the stage-named card sub-buckets (`anytime`/`upcoming`/`someday`/`logbook`/`trash`) — and KEPT on the stage-MIXED / derived surfaces. The `anytime` catalogue is stage-PURE: every Anytime-view member (ANYTIME_SELF — undated-active, arrived-active, arrived someday-scheduled — plus, since R13, the deadline-PULLED undated rows the view now includes) derives `anytime`, because an ARRIVED `startDate` is Anytime not Upcoming (Upcoming is strictly future) and a pull re-files to Anytime, and templates are excluded from the view. `stage` is KEPT on `upcoming` (stage-MIXED — it also carries deadline-forecast `anytime`/`someday` rows), plus `search`, `changes`, the projects/areas listings, the card NODE, and detail. The derived `when` (R12) is dropped inside the `today` view's two `children` buckets (the bucket key states today/evening) and inside a card `upcoming` date-group keyed on it; kept everywhere else it is present (a logbook/trash row is never a Today member and no longer scheduled-forward, so it carries no `when` at all).

**R10 view-card reshape (project-view = read-shape v2 PR 2).** `data.view = { project, children, headings[] }` and NOTHING else at that level. The card node keeps everything (children derive their container from it). `children` is the un-headed BODY's stage-keyed bucket RECORDS: `anytime: {items, total?}`, `upcoming: [{when, items, total?} …]` (the day-block ARRAY, chronological — a dated row under its `startDate`, a template under its projected next occurrence — with a trailing **`{when: null, items}`** resting block for the date-less recurring templates, #V8; a member's `when` drops inside its block, R12), `someday: {items, total?}`, and per-container **`logbook: {items, total?}`** (the body's swept/resolved children, `stopped` DESC). `headings[]` is EVERY heading — open, archived-unswept, AND archived-SWEPT — in `index` order (R5), each `{uuid, title, archived?, children}` with the SAME recursive `children` shape; a swept heading's logged children ride ITS `children.logbook`, and an anomalous open child (HEADARC2-C) rides its `children.anytime`, the heading's `archived` mark making the anomaly self-evident (#V12). Each of the four buckets is stage-pure → `stage` drops in all of them (the bucket key states it); every bucket row also drops `project`/`area`/`heading` (R6). Deleted vs. the v1 wire: `logbookHeadings`, the root `logbook`, and BOTH advisory keys (`openChildrenWhileResolved` / `openChildrenUnderArchivedHeading`) — the GUI's unified logbook region and the odd-child advisories are TTY-derived from the per-container `children.logbook`, never encoded (R6/#V12). `total` is present INLINE iff a bucket was capped (R1); the project view is uncapped today, so every `total` is absent (the inline-truncation sweep is migration PR 5).

**R10 view-card reshape (area-view = read-shape v2 PR 3).** `data.view = { area, children, projects }` and NOTHING else at that level. `area` is the area NODE (tags folded to string names), or **`null`** for the `loose` pseudo-area. `children` is the area's DIRECT to-dos as THREE stage-keyed bucket records — `anytime: {items, total?}`, `upcoming: [{when, items, total?} …]` (the day-block ARRAY, chronological, with the trailing **`{when: null, items}`** resting block for date-less recurring templates, #V8), `someday: {items, total?}` — and **NO `logbook` key**: an area has no logged-children region, and its logbook is the bounded query `things logbook --area <ref>` (#346). `projects` is the child-project sidebar-rank scope as ONE bucket record `{items, total?}` — a mixed-stage listing whose rows KEEP `stage`/`when` (the someday-projects / active split is TTY-only) and drop `area`. Each direct-to-do bucket is stage-pure → its rows drop `stage` (and `area`, and have no `heading` — R6/R7: a someday/anytime direct to-do with a deadline seats in its canonical stage bucket, never also in a day block, so no uuid appears twice). `total` is present INLINE iff a scope was capped (R1): only `children.anytime` (the `--area-limit` scope) and `projects` (the `--project-limit` cap on the ACTIVE project rows) can carry one — the scheduled/someday direct-to-do blocks and scheduled/someday project rows always survive. This RETIRES the `meta.truncation.blocks[]` sidecar for this view (its whole-view `shown`/`total`/`limit`/`truncated` rollup survives); the TTY per-section footers are unchanged (derived from the same bounded view).
