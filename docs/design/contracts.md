...
- **Eligibility:** `tempId` is valid ONLY on a uuid-minting op — `todo.add`, `todo.add-logged`, `project.add`, `project.create-repeating`, `area.add`, `heading.create`, `todo.duplicate`, `todo.make-repeating`, `project.make-repeating`, `todo.convert-to-project`, `heading.convert-to-project`. It is a usage error on `tag.add` (tags have no uuid — reference a tag by its title) and on any non-minting op.
