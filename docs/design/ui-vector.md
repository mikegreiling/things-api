...
This is the transform-loud half of the **hybrid create-quiet / transform-loud doctrine**: keep create and edit on the quiet vectors (tier 0, no focus steal), and reach for GUI driving ONLY for the transforms with no other spelling. Ratified by Mike 2026-07-14: **BUILD, Accessibility-API-only, closet-mini target.** **Eleven ui-drive ops ship** (`UI_DRIVE_OPS`, `src/write/operations.ts`) — plus the `project.add-repeating` composite that rides `project.make-repeating`. Certification has since RUN in the VM lab (UIC1/UIC3/UIC5/UIC6/HEADCERT1): **all 11 are LAB-CERTIFIED** (run end-to-end through the real pipeline in a VM clone with the DB deltas asserted) — HEADCERT1 (2026-07-17) closed the last one, `heading.convert-to-project`, by revealing the heading's parent project and selecting the heading row by position (the `things:///show` reveal UIC1 tried selects to-dos only). On-device `certified` confirmation on target hardware is still pending ([ui-certification-runbook.md](../lab/ui-certification-runbook.md)). The manifest **`src/write/vectors/ui-certification.ts` is the per-op ground truth**; see [Certification-status machinery](#certification-status-machinery) below and "The ops" section for the current roster.
...
### The cost model, and the Repeat dialog's shape manifest (RDLAT2)

**A drive's portable cost is what it asks of the tree, not how many osascript hops it takes.** A hop's wall time on a lab clone is mostly its process spawn (~64 ms) and says almost nothing about the same hop on a real machine. There are TWO portable terms, and the field measurements name them separately:

- **ELEMENTS REALIZED** — the app materializing a view's content when Accessibility asks what is in it. Measured on the maintainer's M1 (sidebar probe, 2026-09-02): **~115 ms per element**, paid on the first content-bearing touch (AXChildren / AXDescription / AXValue), paid AGAIN on a repeat sweep because the app discards what it realized, and **independent of tree depth and of call count** — a 174-row sweep cost ~20 s at 1,841 calls and ~20 s at 2,081 calls. Geometry (`position`, `size`) and `role` are free.
- **ROUND-TRIPS** — Apple events to System Events. The IPC itself is cheap (0.12 ms a read through the JXA bridge, 0.05 ms native — **a native helper-side AX driver is not justified by this data**), but each event is a synchronous trip serviced on the app's main thread.

Which term dominates depends on the surface, and the two must not be conflated: a PLURAL read touches N elements in ONE event, so an event count under-reports the first term exactly where it is largest. The driver therefore reports both. `axElems` — distinct controls whose content was read — rides each hop's own stderr and needs only `THINGS_API_TRACE=1`, so it survives deputy routing; `axOps` — Apple events — needs `THINGS_API_AX_COUNT=1` and the spawn in the CLI's own process. Both land on the hop's trace record beside `durationMs` (`src/write/vectors/ui.ts`).

**For the Repeat dialog specifically, the element term is bounded by the dialog's size.** It is 12 controls wide, 22 at its widest — against 174 sidebar rows — so realizing the whole of it once is 1.4–2.5 s even at the field's 115 ms, and measured on a clone its content reads cost exactly what its geometry reads do with no realize-and-discard signature (RDLAT2 §E). The sheet is not the sidebar, and a model built for one should not be assumed for the other: a single field trace now settles it, because one run carries both counts. It rides Apple's `AEDebugSends`, which writes to **stdout** ahead of the script's own result — so the seam counts and STRIPS the diagnostic from both streams, and an armed count can never change a verdict or a refusal sentence. It is off by default, and on a helper-routed host the variable has to be in the DEPUTY's environment for the count to appear (absent, `durationMs` still lands).

Three rules follow, and each is measured in [RDLAT2](../lab/rdlat2-repeat-dialog-latency.md):

1. **Read a container's controls in the PLURAL, and scan it FEWER TIMES.** These are two different wins and only the second one moves the element term. `value of static texts of <g>` is one Apple event whatever the control count, where the per-control loop is one each — that is the round-trip win. Handing the settled snapshot to every rule that used to go and re-read the group takes the cadence group from FOUR full scans per step to two — that is the element win, and it is the one a field host pays 115 ms a control for. Same laws, one instant, and every addressing decision made on the read that was proven stable.
2. **Ask for a role LIST, not a count per class.** The dialog shell's control census was five `count of <class>` reads (43.5 ms) and is one `role of UI elements` list (6.5 ms); System Events derives each of those classes from exactly that `AXRole`, so the numbers are identical by construction.
3. **Do not gather decoration you will not print.** The census's focus probe is its most expensive read and decides nothing while Things owns the screen — the guard renders the focused role only when refusing, and it refuses on Things NOT being frontmost. The OPERATIONAL census (`decorate: "when-not-ours"`) therefore skips it in that case; the DIAGNOSTIC census (`doctor --ui-state`, rescue, the cleanup disclosure) keeps every probe. The secure-system-modal signal is unaffected: a consent dialog owns the screen, so the probe still runs whenever one is up.

**The shape manifest** (`src/write/vectors/ui-shape.ts`) is what makes a remembered shape safe here. The Repeat dialog does not have a tree — it has one PER STATE (ticking "Add deadlines" mints a shell text field, #646; an ends bound inserts a numeric field ahead of the interval, #589; a frequency switch rebuilds the cadence group, BEEP1). So the manifest holds expectations, in two forms with two postures:

- **The SHELL census is an ASSERTION, checked once at the `dialog-open` hop.** That hop waits for the dialog and censuses it in the same script, reporting which of the two shells opened and its direct-child roles. A shell that does not present the measured census (two checkboxes, one direct pop-up, two buttons, one group, at most one direct text field — CGRD1 §B, and identical for the detached editor) is a REDESIGNED dialog, and the drive refuses rather than pressing structural indices into it. A shell that matches is then ADDRESSED by every later step instead of re-discovered, and the pre-commit audit needs no resolution hop at all.
- **The CADENCE-GROUP shape is ADVISORY, and asserts only what the step can know.** The settle exists to wait out a group being rebuilt, and agreement alone is the absence of movement — which is also what a group looks like before the step's own input has landed. So where the manifest can name the finished state the settle WAITS for it rather than merely accepting it. What it names is deliberately partial: after-completion is *neither anchor label and one field*; an ends-after bound is *two fields* (the step just inserted the second); any other fixed frequency is *both anchor labels and no claim about the count*, because a reschedule opens pre-populated and may already show the ends field. That last case was originally given no expectation at all, on the reasoning that fixed→fixed is indistinguishable either side — true, and the wrong conclusion: `make-repeating` always opens on the dialog's after-completion default, so waiting for `Every` and `Ends:` to appear IS waiting for the rebuild, and without it the interval step discriminated its target against a group that had not rebuilt yet (RDLAT2 §E.4).

**Invalidation is the app version**, read once per process from the bundle's `Info.plist` and matched as a prefix (`3.23` covers its point releases). An unrecognized build asserts nothing, takes no fast path, and runs the full per-step discrimination. The runtime match stays the authority; the version key only stops a known-foreign build being measured against expectations that were never about it.

**One law the campaign had to learn the hard way:** a settle whose interval is "however long two reads happen to take" is a timing dependency wearing a closed loop's clothes. Making the reads cheap broke the fixed-frequency interval step deterministically, because the rebuilt field reports a stable shape for a while before it will accept keyboard focus. The remedy is never to slow the reads down — it is to wait for the observable itself. The typing loop now asks for focus, looks, and tries again on its next attempt; nothing is typed without proven focus, and a field that never accepts it still refuses in the same words once the attempts are spent.
...
**Eleven built ops**, all `vector = "ui"` — the to-do repeat family (make / reschedule / pause / resume), `todo.convert-to-project` + `heading.convert-to-project`, the PROJECT repeat family (make / reschedule / pause / resume), and `area.reorder` — plus the `project.add-repeating` composite (`project.add` + a `project.make-repeating` promote). **Per-op certification** (ground truth: `src/write/vectors/ui-certification.ts`): all ten of `todo.make-repeating`, `todo.reschedule-repeat`, `todo.pause-repeat`, `todo.resume-repeat`, `todo.convert-to-project`, `project.make-repeating`, `project.reschedule-repeat`, `project.pause-repeat`, `project.resume-repeat`, and `area.reorder` are **LAB-CERTIFIED** (UIC1 / UIC3 / UIC5 / UIC6; `project.add-repeating` rides `project.make-repeating` and is likewise lab-certified); **`heading.convert-to-project` is now LAB-CERTIFIED too** (HEADCERT1 — reveal the parent project, select the heading row by position). All eleven ops are lab-certified. `todo.stop-repeat` was **dropped** (never worked — Stop is card-only, UIC2-d), and there is **no `project.stop-repeat`** (the project Stop then selecting the demoted project crashes Things — CRASH1 / §7 C5).
...
| `repeat` `{scope: project, action: add}` | `project.add-repeating` composite | `title`, `frequency`, `interval`, optional `area` |
