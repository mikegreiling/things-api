...
Since the vector is uncertified, this is the **intended** addressing. Most steps are **title-pinned English**; AXIdentifier is **preferred wherever the sitting finds the app exposes one** (the discovery pass captures these — see the runbook). "dynamic" steps are async and covered by wait-for-element rather than the canary.
...
**Certification is now a lab operation (AXVM1).** The original plan assumed AX was unprobeable in the VM lab (SIP), making a real-hardware sitting the *only* path. **AXVM1 (PR #136) falsified that**: Accessibility is grantable in a Tart guest, so the certification suite — the AXVM1 grant recipe plus the seven ops run against the seeded library, asserting the expected DB deltas — is **runnable in a VM clone per Things version**, and can be folded into `lab:regress`. The real-hardware sitting ([ui-certification-runbook.md](../lab/ui-certification-runbook.md)) becomes a **final confirmation on target hardware**, not the sole route. Wiring that in-VM run for the tier-1 ops is the immediate follow-up (see up-next §2); the manifest ships all-uncertified until it lands.

- **Manifest:** `src/write/vectors/ui-certification.ts` records, per op, its `status` (`uncertified` | `certified`) and the evidence ids backing it (UI2-a make-repeating, UI2-b reschedule, UI2-c pause/resume, UI2-d convert, UI2-i stop). The certification sitting flips entries and adds the new (real-hardware) evidence ids.
