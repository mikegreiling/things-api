...
**A drive's portable cost is ACCESSIBILITY ROUND-TRIPS, not osascript hops.** A hop's wall time on a lab clone is mostly its process spawn (~64 ms) and says almost nothing about what the same hop costs on a real machine; the round-trip count says the same thing on every host. Measured 2026-09-02 on the maintainer's M1: **~20 ms per AX call against ~1.7 ms on a clone.** So the number to minimize is how many questions a hop asks the tree, and the driver reports it: with `THINGS_API_AX_COUNT=1` armed, every traced hop carries `axOps` beside `durationMs` (`src/write/vectors/ui.ts`). It rides Apple's `AEDebugSends`, which writes to **stdout** ahead of the script's own result — so the seam counts and STRIPS the diagnostic from both streams, and an armed count can never change a verdict or a refusal sentence. It is off by default, and on a helper-routed host the variable has to be in the DEPUTY's environment for the count to appear (absent, `durationMs` still lands).

Three rules follow, and each is measured in [RDLAT2](../lab/rdlat2-repeat-dialog-latency.md):

1. **Read a container's controls in the PLURAL.** `value of static texts of <g>` is one Apple event whatever the control count; `value of static text i of <g>` in a loop is one per control. The cadence group's whole inventory — static values and positions, field values and positions — is four events, and every addressing law is then computed from that one snapshot rather than from a series of reads taken over ~70 ms. Same laws, one instant, a quarter of the round-trips.
...
- **The CADENCE-GROUP shape is ADVISORY, and supplied only where it DISCRIMINATES.** The settle exists to wait out a group being rebuilt, so letting a shape match end that wait is sound only when the shape is one the previous state could not also have had. After-completion (neither anchor label) and ends-after (two numeric fields) qualify; a fixed frequency switching to another fixed frequency looks identical either side, so the manifest says nothing there and the two-agreeing-reads rule decides alone. Where an expectation exists the settle WAITS for it and refuses if it never arrives — strictly stronger than agreement, which is only the absence of movement and is also what a group looks like before the step's own input took effect.
