...
| `anytime` (global) | `data.sections = [ { area, items, total? } … ]` | record-ized only. Order-axis grouping (#C2): area sidebar rank; NOT reorder scopes (a projection across containers — the GUI's own drag here is secretly a compound reorder+heading-reparent via adjacency inheritance, see oddities; any API counterpart is a future *explicit* compound op, never an implicit reorder semantic — #V9 proposed) |
...
- **#V9 (PROPOSED, ratify with this doc)** — the global anytime/someday flat projections stay non-scopes; the GUI's drag-sort there (dragged row inherits the heading of the row above the drop target — maintainer-observed, recorded in oddities) is a compound reorder+reparent. If the API ever offers it, it is an explicit compound operation with disclosed reparenting, never an implicit `reorder`.
...
Living-doc updates ride each PR per AGENTS.md. On ratification, this doc's status flips to RATIFIED and v1 gains a "superseded by v2" banner. (HEADSORT's verdict is already folded in — §2 R5, ruling #V7; the capability matrix and assumption register were updated by the probe PR #400.)
