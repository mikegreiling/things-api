...
4. **Verification**: post-state checks via the existing SQLite verify machinery (state read → compare). heading.add returns the new heading's uuid (query by project + title + max index; handle duplicate-title candidates conservatively — newest creationDate).
...
- Whether `heading.add` should accept a position/index (only if the proxy supports it; do not invent).
