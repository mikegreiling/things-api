...
| `project.move-heading-to-project` | `things project move-heading-to-project <project-ref> <heading-sel> --to <project-ref>` | the CROSS-PROJECT relocation (children follow via their heading FK); ui-vector, GUI-only (HEADXPROJ recipe, lab-certified HXPC1) — see below |
...
**Cross-project heading move — `project.move-heading-to-project` (SHIPPED, ui-vector, lab-certified HXPC1 2026-07-28).** No headless spelling exists on any vector (URL ⛔, AppleScript `move → project id` 301, Shortcuts ⛔), so it is delivered through the Accessibility ("ui") vector: the heading row's `…` ellipsis (`AXUnknown desc="More. <title>"`, HID-clicked because `AXPress` is inert) → the `Move…` popover item → a search-by-title project picker (type the destination, Return). Two-key gated (`ui.enabled` + `dangerouslyDriveGui`), disruption tier 3 (foreground-bound mouse synthesis). Verify oracle (HXPC1): the heading row's `project` FK becomes the destination; its children are UNCHANGED and follow via their intact heading FK — a single-row change.

**Kept a SEPARATE VERB, not `--to-project` on `move-heading`.** `move-heading` is a pure within-project reorder (native, undoable, cheap); the cross-project relocation is a distinct, GUI-only, container-crossing operation with its own gating, refusals, and (lack of) undo. Folding it in as a flag would conflate a safe in-place shuffle with a destructive-ish move and hide the two-key GUI cost behind an innocuous verb. Two collision surfaces fail closed (the drive addresses BOTH the source heading and the destination BY TITLE): a source heading title shared by another heading in the project, and a destination title shared by another project (the picker is search-by-title — Return would land the wrong row). A titleless heading is refused (its `"More. "` description is not unique). **Undo: none wired** — the move IS app-reversible (move it back with the projects swapped), but re-driving a GUI op on undo is deferred; `undo` reports it irreversible and names the manual inverse. The `project.move-heading` selector rules (title-or-uuid, duplicate-title-fail-closed, uuids stay valid) apply to `<heading-sel>` here too.
...
5. **Placement honesty.** "Top of bucket in selection order" is guaranteed where a reorder protocol exists for the destination bucket, and app-default placement with an explicit note in the result where none does. The result payload states which you got. No silent pretending. **Bucket classification per the REORDGAPS + BOUNCE2 verdicts (2026-07-27, [../lab/reordgaps-results.md](../lab/reordgaps-results.md)), as WIRED in the Phase A.1 build — the "protocol-proven, not-yet-wired" bucket is dissolved:** GUARANTEED — loose inbox/today/evening/someday-view/**area-less loose anytime** (ANYBNC reverse-order bounce); a project's unheaded anytime AND someday children (SOMEORD-b native, plus the SOMEBNC-project forward-order bounce fallback when native is unavailable); an area's anytime members; **an area's someday members** (SOMEBNC-area reverse-order bounce — the planner uses the bounce, NEVER the destructive §9f area reorder command); **a heading's anytime children** (BOUNCE2-h forward-order back-insert bounce); **a container's same-day scheduled children** (DAYORD-b native `todayIndex` re-rank, date-preserving, private-surface-gated); area-less someday/anytime projects. WITHIN-HEADING `--before`/`--after` against an unmoved sibling is now SUPPORTED too (see rule 6 — the extended bounce co-bounces the members between the block and the anchor, disclosed). APP-DEFAULT-WITH-NOTE — a headed scheduled/someday/evening sub-bucket, a project/area child's evening sub-bucket, a loose future day (`list "Upcoming"` re-dates, §9g), and repeating TEMPLATE movees (the private reorder no-ops on template rows, §9e addendum; the planner reports them explicitly, never silently). REFUSED (no protocol at all) — the only remaining `--before`/`--after` refusal class.
6. **Compilation.** Both verbs compile onto the lab-locked wire protocols (native re-rank, anchor-stack two-call, bounce) through a minimal-move planner: fewest wire legs for the requested placement, per-scope caps apply. **Bounce direction follows the BOUNCE2 re-entry law** — loose/area-direct items FRONT-insert (reverse-order legs), strict-container children (heading/project) BACK-insert (forward-order legs); both are deterministic and state-preserving. An ANCHORED bounce (`--before`/`--after`) co-bounces the minimal contiguous run between the block and the bucket edge (a back-insert forward-bounces the block-through-end suffix; a front-insert reverse-bounces the top-through-block prefix), counting TOUCHED items against the cap and disclosing the co-bounced-but-unnamed siblings in the result. **Verify: ONE assertion per item round-trip** — the transient someday state of the AWAY/BACK legs is not independently verified; the orchestrator asserts the when-restore + position delta at each item's completion (fail-loud retained). The bounce abort-honesty model (placed vs remaining) applies throughout.
...
| `--before`/`--after` into a bucket with NO reorder protocol at all (a headed scheduled/someday sub-bucket, a project/area evening sub-bucket, a loose future day, a repeating template) | `unsupported`/`blocked` — no honest positioning exists there. NOTE: within-heading `--before <unmoved sibling>` is NO LONGER refused — the extended bounce co-bounces the intervening members (disclosed) |
| Bounce cap exceeded — the count of TOUCHED items (block + co-bounced siblings for an anchor placement) exceeds `bounce-max-items` (default 30) | cap error citing the configured value and the touched count |
| A bounce-dependent placement while `bounce-enabled=false` | `unsupported`/app-default-refused — teaching error naming the flag; NEVER a destructive or unverified fallback |
