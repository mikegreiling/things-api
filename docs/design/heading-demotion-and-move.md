...
5. **Placement honesty.** "Top of bucket in selection order" is guaranteed where a reorder protocol exists for the destination bucket, and app-default placement with an explicit note in the result where none does. The result payload states which you got. No silent pretending. **Bucket classification per the REORDGAPS observed verdicts (2026-07-27, [../lab/reordgaps-results.md](../lab/reordgaps-results.md)) — this supersedes the pre-campaign expectations:** GUARANTEED — loose inbox/today/evening/someday-view; a project's unheaded anytime AND someday children (SOMEORD-b); an area's anytime members; area-less someday/anytime projects. APP-DEFAULT-WITH-NOTE, upgradeable — a container's same-day scheduled bucket (DAYORD-b found a date-preserving `todayIndex` spelling) and area-less loose anytime to-dos (ANYBNC bounce): both protocols are now PROVEN but wiring them as guaranteed requires touching the shared lab-locked reorder op — queued as a scoped Phase A.1 follow-up, shipped honest-noted meanwhile. HEADING BUCKETS — membership + top-of-heading guaranteed (the bounce preserves the heading FK, HEADORD-c); multi-item in-heading ORDER is now DETERMINISTIC and promisable (BOUNCE2-h, 2026-07-27, resolved the HEADORD-c anomaly: the bounce BACK-inserts a headed child — appends to the heading-bucket end — so a FORWARD-order bounce lands the exact requested order; protocol-proven, Phase A.1 wiring); only in-heading `--before <unmoved sibling>` anchoring stays REFUSED (HEADORD-b proved no heading-as-container spelling exists — a full-block reorder is fine, an anchor relative to a sibling that isn't moving is not). WITHIN-AREA SOMEDAY ORDER — REFUSED via the area REORDER command (it DE-SOMEDAYS them, §9f) but PROTOCOL-PROVEN via the `when=` bounce (SOMEBNC-area, 2026-07-27: reverse-order front-insert, `start=2` + area FK preserved) — the planner uses the bounce, never the destructive command. REFUSED, destructive — `list "Upcoming"` (re-dates, §9g). NO-OP-AWARE — repeating template movees (the private reorder no-ops on template rows, §9e addendum; the planner reports them explicitly, never silently).
