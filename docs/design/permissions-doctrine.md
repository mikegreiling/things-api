...
| both ceremonies at a TTY | the wizard: one explainer per dialog that is actually coming, an Enter gate between legs, and the tier question when nothing else decided it. Off a TTY every method is inert — strict mode is byte-for-byte what it was | `src/wizard.ts`, `src/direct-setup.ts`, `src/deputy/install.ts` |
