...
Under `--json` the synonymy is total in the payload: `things projects <ref> --json` emits the **same `kind` and the same `data`** as `things project show <ref> --json` — the detail view, with the project's rows under `data.view.children` (`anytime` / `upcoming` / `someday` / `logbook`) and `data.view.headings[].children`, never a flat `data.items`. The list envelope's flat `items` array belongs to the *bare* plural (`things projects --json`), which is a different view. The only envelope difference between the two ref forms is `meta.resolvedCommand`, present on the plural and absent on the canonical singular. Pinned by `test/cli/e2e.test.ts` ("the plural project synonym emits the SAME --json data as the singular show").
...
### `--dry-run` doctrine (Mike-approved) — accepted by EVERY command

`--dry-run` is a universal flag on the WHOLE surface, not just the write verbs: every `things` command accepts it, root included, with one contract — **"guarantee nothing changes"**. Behavior by command class:

- **DB-mutation verbs** (the todo/project/area/tag write family, `batch`, `undo`, `reorder`) preview the planned change and its expected effect and execute nothing — the original meaning, unchanged.
- **Pure reads** (every list view, `show`/`search`/`logbook`/`trash`, `config get`, `capabilities`, `doctor`, `snapshot`, `legend`, `help`) accept it and IGNORE it: output is **byte-identical** with and without it, `--json` and human alike — the normal view IS the correct dry-run of a read, so there is no banner or meta stamp.
- **Local-side-effect commands** (`config set`, `install-skill`, `setup`, the `open`/`todo open`/`project open`/`area open` reveals) do NOT silently ignore it — they honor it with an honest preview that describes what would change and performs nothing (`config set` prints `DRY RUN would set …`; the `open` family returns the URI it would open without foregrounding the app). `mcp` is genuinely un-previewable (it serves a live surface whose tools mutate on request), so it REFUSES `--dry-run` loudly (exit 2) rather than launch a server that ignores the promise.

**Mechanism.** A single decorator (`src/cli/dry-run.ts`, applied last in `buildProgram`) walks the whole registered tree and adds a hidden `--dry-run` to every LEAF that does not already declare one — so a future command inherits it automatically. It is declared on leaves ONLY: commander treats a parent's option as global and would swallow the flag before a child could parse it (which for a write child would silently turn a dry-run into a real change), so the root, the namespace groups, and the `trash` view (a read that also parents `trash empty`) never declare it. On those non-consuming nodes the flag is a pure no-op, so the resolver strips the inert token before commander parses (`stripInertDryRun`) — `things --dry-run`, `things todo --dry-run`, and `things trash --dry-run` all behave exactly as the flag-less form. A completeness lock (`test/cli/universal-dry-run.test.ts`) walks the tree so a future command cannot regress the invariant. The one root-help line states the contract; per-command help stays quiet (the leaf option is hidden).
...
**Object KIND per axis (single-kind index rule).** An INDEX axis — a stage list (`anytime`/`someday`/`inbox`) or a container ref — sorts each object kind in its OWN rank space, so all movees must be the same kind; a mixed to-do + project set is refused naming each movee's kind, *even when they share a container* (an area's someday to-dos and someday projects are different index buckets). The GLOBAL axes — `today` / `evening` and the day-block tokens (`upcoming` / `<YYYY-MM-DD>`) — intermix to-dos and projects on the one `todayIndex` axis (O12 / SIT4 / PROJDL-2c), so a mixed set is legal there. Because a cross-kind set has no shared container index, it is single-axis: a BARE reorder auto-routes to its global (view/day) axis rather than raising a dual-axis refusal. A **heading** carries no stage/schedule/day order of its own — it is never a member of any of these buckets — so a heading movee is refused on every token, pointing at `things project move-heading`. The single-kind gate lives in `runInPlaceReorder` (upstream of the axis resolver); the day/Today intermix relaxation applies only when no `--in` forces an index axis and every movee is a today/evening/scheduled/forecast member.
