...
- **Local-side-effect commands** (`config set`, `install-skill`, `setup shortcuts`, the `open`/`todo open`/`project open`/`area open` reveals) do NOT silently ignore it — they honor it with an honest preview that describes what would change and performs nothing (`config set` prints `DRY RUN would set …`; the `open` family returns the URI it would open without foregrounding the app). `mcp` is genuinely un-previewable (it serves a live surface whose tools mutate on request), so it REFUSES `--dry-run` loudly (exit 2) rather than launch a server that ignores the promise.
