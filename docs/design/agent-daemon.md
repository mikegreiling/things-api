...
- **Signing posture (updated 2026-08-21, maintainer-ratified; supersedes the original two-tier plan):** the maintainer holds a paid Apple Developer account (Developer ID Application, Team VNJWARH2W7). Identity preference in `build-helpers.sh`: **Developer ID Application > Apple Development > self-signed ceremony cert**, always hardened-runtime + timestamped; **ad-hoc is never used** (identity-less; TCC grants can't follow it; amfid kills sandboxed ad-hoc code outright). Cert-class facts that shape everything: an Apple-issued chain is MANDATORY for the sandboxed reader (amfid −423 refuses ad-hoc AND self-signed roots); Apple Development certs are free (any Apple ID via Xcode) and renew yearly with a stable CN; Developer ID lasts 5 years and is the only class Gatekeeper accepts for distributed apps + the prerequisite for notarization; already-signed binaries keep running after their cert expires (secure timestamp); the 7-day-expiry folklore is iOS provisioning profiles, not macOS signing. Self-builders therefore need: nothing extra for the deputy (ceremony cert suffices), a free Apple ID for the reader.
- **Distribution (ratified; SHIPPED 2026-08-23): notarized prebuilt helpers ship INSIDE the npm tarball**, with `things helpers install` as the explicit opt-in activation — no dmg, no separate download page, no project split. CI signs at release time via the standard encrypted-secrets pattern (base64 p12 + passphrase imported into a throwaway keychain in a macOS job; notarization via `notarytool` with an App Store Connect API key secret). Signing runs ONLY in tag-triggered release workflows — fork PRs never see secrets (GitHub withholds them by design). Source + build script remain the bring-your-own-identity path. As built: `.github/workflows/release.yml` job `build-helpers` (macos-14) → tar artifact → the `publish` job unpacks it to `deputy/prebuilt/` and asserts it reaches the tarball; `helpersDefaultBuildPath()` resolves `deputy/prebuilt` ahead of `deputy/build`; the maintainer's one-time secret setup is [release-signing.md](release-signing.md). Still unmeasured: whether a FREE Apple Development cert satisfies amfid for the self-builder's reader (parked in up-next on an interactive Apple ID sign-in). **Single-bundle packaging SHIPPED 2026-08-22** (pulled forward before any deputy Automation grants existed, so the TCC identity flip was free): `Things API Helper.app` houses things-deputy as its main executable and the sandboxed reader nested at `Contents/Helpers` with its own untouched bundle identity (the bookmark grant survives); `things helpers install` owns its `bin/` directory wholesale — a fresh copy every install — which is the whole migration story (no legacy-layout logic anywhere).
- **Secrets hygiene (public repo):** no certificate, key, or Apple credential ever enters the repository — only repo-settings secrets and the scripts/documentation.
...
| privilege | security-scoped bookmark to the Things group container, minted once by `things helpers grant` (NSOpenPanel presented BY the sandboxed process — that is what makes it durable; the panel opens inside the Things folder, and the ceremony verifies the database resolves inside the granted scope) | TCC Automation/Accessibility grants (durable classes) |
...
### 3c. Helper UX (items 1–3 ratified 2026-08-21/22, SHIPPED 2026-08-22)

1. **Auto-detect routing — SHIPPED.** Installation IS the intent signal: `helpers-enabled` is tri-state `auto | true | false`, default `auto`. The matrix is the design (src/deputy/routing.ts carries it as a table): under `auto` a present-and-healthy helper is used, an INSTALLED-but-unserving one degrades to direct LOUDLY (one stderr notice + the doctor section), and an ABSENT one degrades SILENTLY — a machine that never onboarded is not a degraded machine, and a fresh install must not nag. `true` keeps the stricter reading (absence itself is noticed); `false` never routes. Installation is read off the installed bundle on disk, not the socket, so a stopped launchd job still counts as installed. `--helpers`/`--no-helpers` and `THINGS_API_HELPERS` (which now takes all three values) stay the per-invocation escapes.
2. **Doctor awareness + onboarding notices — SHIPPED.** `things doctor` has a `── Helpers ──` section: mode + what it RESOLVED to per half this process, bundle installed + version vs `EXPECTED_HELPERS_VERSION`, launchd registration, liveness, signing, the reader's grant, hung-socket detection (socket present, no handshake), and one remedy line. Prompt-free — it reuses the `helpers status` probes. Passive stderr notices ride human command paths (skill-check precedent, `src/cli/helpers-check.ts`): a version-skew line whose remedy is a rebuild + `things helpers install` (the installed bundle is a COPY, so a restart would only restart stale code), and a throttled once-a-fortnight introduction on a machine that has never answered the routing question and has nothing installed. One notice per process across both producers (`src/deputy/notice.ts`), never on `--json`/`mcp`/`things helpers`, kill switch `THINGS_API_NO_HELPERS_CHECK=1`. Nothing in the helpers CLI path writes inside the package directory, so it already works from a read-only root (`npx`); shipping PREBUILT binaries in the tarball remains the distribution batch's job.
3. **Graceful upgrade drain — SHIPPED (helpers v1.1.0).** Both halves drain on SIGTERM: unlink the socket (no new client can find a dying helper — deterministic where waking a thread blocked in `accept()` is not), close the listener, wait out the requests already dispatched within a 10s bound, flush the audit log, exit 0. SIGKILL stays the hard stop. `launchctl bootout`/`kickstart -k` block on that exit, so the CLI's `launchctl` timeout was raised past the drain bound. Certified live: an in-flight `osascript` (5s stub) SIGTERMed mid-run still answers, then the process exits 0 with its socket removed (`test/deputy/broker-integration.test.ts`).
4. **launchd socket activation + idle-exit** (δ polish): launchd owns the sockets, helpers spawn on demand and may idle-exit; also retires the socket-present-but-dead failure class. The reader regains effective residency automatically once watch clients hold long-lived connections.
5. **β resident worker is a NODE process:** the survive-the-caller engine host runs the same TypeScript library under launchd, using the reader for reads and the deputy for automation — so it needs ZERO TCC identity of its own, the audit ledger stays engine-written, and the dumb helpers never learn what an operation is.
6. **Full onboarding ceremony — SHIPPED (helpers v1.2.0).** `things helpers grant` fires every consent the pair needs in ONE sitting (reader read grant, Automation → Things, Automation → System Events, Accessibility, bundled-shortcuts census) and verify-reports per leg. Each leg is gated on a prompt-free check the deputy's `hello` now carries (`axTrusted`, `automation.{things,systemEvents}`), so a rerun on an onboarded host raises nothing and reports all-green. Design of record: [helpers-onboarding.md](helpers-onboarding.md).
...
| **β1 — permission deputy** (SHIPPED 2026-08-19, §3a; reshaped into the helpers pair §3b, single bundle 2026-08-22) | dumb privileged broker + launchd install + signing scripts + routing with direct fallback + `things helpers status/install/grant/restart/uninstall` | mock-broker suites (all platforms) + live-binary suite on CI macOS; live on the maintainer's host (Developer ID, reader granted) |
