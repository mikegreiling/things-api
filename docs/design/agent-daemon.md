...
### 3c. Helper UX follow-ups (maintainer-ratified 2026-08-21, queued in up-next)

1. **Auto-detect routing:** installation IS the intent signal — `helpers-enabled` becomes tri-state `auto | true | false`, default `auto` = a present-and-healthy helper is used; degradation is always LOUD (one stderr notice + doctor detail), never a silent fallback. `--no-helpers`/env stay as per-invocation escapes.
2. **Doctor awareness + onboarding:** `things doctor` gains a helpers section (installed/loaded/running/granted/signing/version-skew, hung-socket detection); passive stderr notices steer toward installing the helpers + the agent skill when absent, and toward `things helpers install` when the installed helper trails the package version (skill-check precedent; the remedy is a reinstall — the installed bundle is a copy — not a restart). Works under `npx things-api` once prebuilt binaries ride the tarball.
3. **Graceful upgrade drain** (maintainer-requested 2026-08-22): install/restart currently bootout a helper mid-flight — a request in progress dies with the old process. Add a drain step (stop accepting, finish in-flight requests within a bound, then exit) so upgrade-while-busy is safe.
