...
- **Signing posture (updated 2026-08-21, maintainer-ratified; supersedes the original two-tier plan):** the maintainer holds a paid Apple Developer account (Developer ID Application, Team VNJWARH2W7). Identity preference in `build-deputy.sh`: **Developer ID Application > Apple Development > self-signed ceremony cert**, always hardened-runtime + timestamped; **ad-hoc is never used** (identity-less; TCC grants can't follow it; amfid kills sandboxed ad-hoc code outright). Cert-class facts that shape everything: an Apple-issued chain is MANDATORY for the sandboxed reader (amfid −423 refuses ad-hoc AND self-signed roots); Apple Development certs are free (any Apple ID via Xcode) and renew yearly with a stable CN; Developer ID lasts 5 years and is the only class Gatekeeper accepts for distributed apps + the prerequisite for notarization; already-signed binaries keep running after their cert expires (secure timestamp); the 7-day-expiry folklore is iOS provisioning profiles, not macOS signing. Self-builders therefore need: nothing extra for the deputy (ceremony cert suffices), a free Apple ID for the reader.
- **Distribution (ratified): notarized prebuilt helpers ship INSIDE the npm tarball**, with `things deputy install` as the explicit opt-in activation — no dmg, no separate download page, no project split. CI signs at release time via the standard encrypted-secrets pattern (base64 p12 + passphrase imported into a throwaway keychain in a macOS job; notarization via `notarytool` with an App Store Connect API key secret). Signing runs ONLY in tag-triggered release workflows — fork PRs never see secrets (GitHub withholds them by design). Source + build script remain the bring-your-own-identity path. Packaging follow-up (ratified): consolidate both helpers into ONE .app bundle (nested executables with different entitlements — standard practice) so distribution is a single artifact.
...
1. **Auto-detect routing:** installation IS the intent signal — `deputy-enabled` becomes tri-state `auto | true | false`, default `auto` = a present-and-healthy helper is used; degradation is always LOUD (one stderr notice + doctor detail), never a silent fallback. `--no-deputy`/env stay as per-invocation escapes.
2. **Doctor awareness + onboarding:** `things doctor` gains a helpers section (installed/loaded/running/granted/signing/version-skew, hung-socket detection); passive stderr notices steer toward installing the helpers + the agent skill when absent, and toward `things deputy install` when the installed helper trails the package version (skill-check precedent; the remedy is a reinstall — the installed binary is a copy — not a restart). Works under `npx things-api` once prebuilt binaries ride the tarball.
3. **Single-bundle packaging** (with the distribution work in §3).
