...
- **Distribution (ratified): notarized prebuilt helpers ship INSIDE the npm tarball**, with `things helpers install` as the explicit opt-in activation — no dmg, no separate download page, no project split. CI signs at release time via the standard encrypted-secrets pattern (base64 p12 + passphrase imported into a throwaway keychain in a macOS job; notarization via `notarytool` with an App Store Connect API key secret). Signing runs ONLY in tag-triggered release workflows — fork PRs never see secrets (GitHub withholds them by design). Source + build script remain the bring-your-own-identity path. **Single-bundle packaging SHIPPED 2026-08-22** (pulled forward before any deputy Automation grants existed, so the TCC identity flip was free): `Things API Helper.app` houses things-deputy as its main executable and the sandboxed reader nested at `Contents/Helpers` with its own untouched bundle identity (the bookmark grant survives); `things helpers install` owns its `bin/` directory wholesale — a fresh copy every install — which is the whole migration story (no legacy-layout logic anywhere).
