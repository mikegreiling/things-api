...
| Class | Ops |
|---|---|
| reversible | todo.add, project.add, todo.duplicate, project.duplicate, todo.complete, todo.cancel, todo.reopen, project.complete, project.cancel, project.reopen, project.delete, project.restore, todo.restore, todo.set-tags, project.set-tags, area.update, tag.update, heading.rename, heading.unarchive, reorder (native), area.add\*, tag.add\*, todo.edit-checklist-item (TARGETED 3-way-merge inverse against the live list; refuses when the target moved out of band or duplicate-title ambiguity) |
| reversible-with-loss | todo.delete (restores to Inbox de-scheduled, E15), todo.replace-checklist (per-item completion NOW restored via the json form P18 — the T07 "state unrecoverable" caveat is retired; refuses on ANY out-of-band diff; canceled checklist items round to open), todo.backdate (day precision), todo.update (evening bucket for stale items; schedule/reminder entanglement), project.update (same), heading.archive (cascade children reopen), todo.clear-dated-reminder (reminder restored against CURRENT schedule; see below) |
