...
## Addendum — 2026-08-05: `derived.todaySection` + `derived.reminderLive` deleted (vocab arc finale)

The two remaining redundancies flagged in the Batch-2 detail are now gone (ALPHA, entity-model break, no compat machinery):

- **`derived.todaySection` DELETED outright.** It was informationally redundant with the presence-keyed Today markers: `todaySection === "evening"` ⇔ `evening: true`, and `"today"` ⇔ `today && !evening`. Every consumer was rewritten against the markers — the write-verify schedule delta (`whenAssertions` today/evening now assert `{today:true, evening:null}` / `{evening:true}`), `verify/delta.ts` `getField` (name dropped), `undo.ts` (schedule reconstruction + the `start/startDate/today/evening` clobber-guard read the `evening` marker), the today-view evening split (`views.ts` reads the `evening` marker directly — it already carries the exact `startBucket=1 ∧ startDate=today` semantics `todaySection === "evening" ∧ startDate === today` expressed), and the TTY (`render.ts`/`glyphs.ts` never read it — they consume the derived `when`). `mapTodaySection` + `TODAY_SECTION_FROM_DB` + the `TodaySection` type are removed.
- **`reminder` gated at INGESTION; `derived.reminderLive` DELETED.** The top-level consumer `reminder` is now the LIVE value only — null once the byte is presentation-dead (§9n, its `startDate` gone strictly past) — live-gated at the mapper under the response clock (the `reminderIsLive` predicate), exactly where the `today`/`evening` markers are gated. The RAW stored byte (which survives a stale schedule and is revived by re-scheduling) moves into the substrate as `derived.reminder`; the write engine reads THAT for its pre-state/delta predictions (`effectiveReminder` auto-preserve, `H-NO-REMINDER`, `clear-dated-reminder` verify + undo capture, and the `reminder` write-verify assertion, which `getField` resolves to the raw byte). Read-shaping's `reminderLive`-keyed `delete o.reminder` is gone: a null top-level `reminder` is pruned by omit-empty like any other nullable field, so the wire is byte-identical.

**Substrate bag after this change — 6 fields:** `start`, `logged`, `trashed`, `today?`, `evening?`, `reminder` (raw byte). The wire and TTY are byte-identical (proved via the shape/render/e2e/wire-key-inventory suites); the break is entirely on the programmatic entity model.
