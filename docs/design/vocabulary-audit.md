...
Status: **audit LANDED, execution pending.** This is the deliverable for [docs/up-next.md](../up-next.md) §7 item 1 — the classification of record for the "one name per concept" tech-debt pass. It does NOT perform any rename; it maps every consumer-facing key to its internal representation, classifies each, and proposes the mechanical rename batches + the substrate-segregation change that execution would carry out.
