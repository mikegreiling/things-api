...
| **Spawned occurrences** (an after-completion or fixed schedule materializing the next instance) | born at **occurrence-midnight in the SPAWNING DEVICE's local zone**, NOT wall-clock (SYNC2B SY-3 resolved the SERDEL S5 zone residual: UTC device → UTC midnight `1783296000`; America/Chicago device → CDT midnight `1783314000`, +5 h). **Cross-device:** two devices that spawn the same occurrence while disconnected each mint their own instance (own uuid, own device-local midnight), then Things Cloud **dedupes to ONE** on reconvergence — the surviving row's `creationDate` is the winner's device-local midnight (propagated to all devices). | [rsim-results](../lab/rsim-results.md) §RSIM-R; TIMEZ2 early-materialization confirms the same midnight anchor (RD-11); **SYNC2B SY-3** ([sync2b-durable-account](../lab/sync2b-durable-account.md)) |
