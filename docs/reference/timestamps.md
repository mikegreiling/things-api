...
| **`project.dissolve-heading`** surviving children `umd` | **UNMEASURED (residual, cause documented)** | SRCFATE attempted it but the op's ellipsis-`…`-menu drive **did not resolve headlessly** (the More-button position didn't resolve in the golden-v2 framebuffer — a clean `verify-failed:silent-noop`, no change landed), AND the op is currently **CLI-uninvokable** (the command omits `addDriveGuiFlag`; see [up-next.md](../up-next.md)). Needs a visible-framebuffer sitting. Predicted (not asserted) BUMP by the reparent-bump law (WG-2). |
| **`project.move-heading-to-project`** (cross-project) heading `umd` | **UNMEASURED (residual, cause documented)** | Same obstacles as dissolve-heading above (SRCFATE): ellipsis-menu position unresolved headlessly + CLI-uninvokable. Predicted BUMP; not measured. |
