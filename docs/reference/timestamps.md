...
| **AS `set modification date`** | `umd` | **SHIPPED** as the universal **`--preserve-modified`** write flag (`preserve_modified` on MCP; `WriteOptions.preserveModified`) | the timeline-silent-mutation lever (capture `umd` before the write → mutate + verify → restore to `floor(umd0)`; one restore leg per bumped pre-existing target, once after a compound's last leg). Best-effort — a failed restore is disclosed, never fatal; a create-only / `umd`-silent op is a no-op. 1-second resolution floor (restore ≤ original — the safe `changes --since` direction). **sync-gate caveat** — proven only on an UNSYNCED store, its interaction with Things Cloud's timestamp-ordered 3-way merge is UNKNOWN (SYNC2 blocked), so it must not run against a synced production store until probed (the caveat rides the `--help`/skill/doc copy). TAGMOD T5 |
