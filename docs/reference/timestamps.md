...
| **`project.dissolve-heading` / `move-heading-to-project`** children/heading creationDate | **not a creationDate concern** | These re-home surviving rows (no identity replacement) — the rows keep their original `creationDate` trivially. DISS1 captured `index` preserved; HEADXPROJ captured a single-row FK change. (Their `umd` footprint is the open question — see §2c.) |
...
| **`project.dissolve-heading`** surviving children `umd` | **UNMEASURED (residual, cause documented)** | SRCFATE attempted it and the drive died at the `…` button, which it read as a headless-framebuffer limit (a clean `verify-failed:silent-noop`, no change landed). **HXPC1 (2026-08-25) supersedes that reading: the button was addressed one level too shallow and resolved on NO host** — a `whose` clause searches the content table's direct children, its rows, while the button sits two levels below one ([hxpc1-picker-assert](../lab/hxpc1-picker-assert.md) §B0). The path is fixed, so this cell is now measurable in an ordinary clone; it has not been driven yet (HXPC1 certified the move-heading sibling, not this one). Predicted (not asserted) BUMP by the reparent-bump law (WG-2). |
| **`project.move-heading-to-project`** (cross-project) heading `umd` | **UNMEASURED (residual, cause documented)** | Same obstacle as dissolve-heading above, with the same resolution: the `…`-button path was wrong, not the rig (HXPC1 §B0). The op now drives end to end in a clone (HXPC1 cells b/c), so this cell is measurable — it simply was not part of that campaign. Predicted BUMP; not measured. |
