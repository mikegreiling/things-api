...
**[CERTIFIED core · one UNPROBED corner]** — evidence: [glossary](glossary.md) log-boundary entry; plog1/A28/LOGNOW; HEADSORT/LOGSORT flip observations. Implementation: [src/read/log-boundary.ts](../../src/read/log-boundary.ts).
...
- **The Daily→Manually forward-sweep corner [UNPROBED].** A Daily→Manually flip's `manualLogDate` stamp lands at *flip time*, which would forward-sweep that day's still-pending window. Only Immediately→Manually flips have been directly observed; the Daily→Manually transition is the one corner not yet probed.
