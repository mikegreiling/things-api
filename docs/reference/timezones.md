...
**The one dated-evening row that DOES exist — and the gate it needs (STEV1, 2026-08-31, golden-v4 / Things 3.23).** No surface can *write* `startBucket=1` with a non-today `startDate`, but a row can *become* one for free: set an item to This Evening, leave it undone, and let the day roll over. The app never cleans the byte — measured byte-identical across a rollover + relaunch, a Today-view visit, a select-and-scroll and a second relaunch — it re-derives placement at render time instead, drawing the row in Today PROPER (bottom of the daytime section, above the "This Evening" header). So **every** consumer of `startBucket` must apply the day gate itself, against the DEVICE-LOCAL day (`localToday`, never a UTC date): a bucket-1 row is in This Evening only while its `startDate` is exactly today, and is an ordinary Today member otherwise. `things-api` applies it in exactly one place, `todayPlacement` (`src/model/today-placement.ts`), consulted by the read views, both reorder scopes, the reorder bounce's concurrent-edit re-check and the move planner's `--in` axis check — four surfaces that used to spell it separately, which is how #657 ended up with a to-do that neither scope would reorder. Evidence: [lab/stev1-stale-evening.md](../lab/stev1-stale-evening.md); the data half is oddities §9n.
