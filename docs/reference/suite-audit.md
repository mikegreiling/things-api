...
The hardened post-write discovery and the `repeating` response block (`todo`/`project` make-repeating, and `project.create-repeating` via its promote leg) are covered at the unit + engine layer — complementing, not replacing, the per-version ui-vector certification above:
