...
All eight suites plus the guest write-layer e2e **in both arms** — `direct` on a golden-v5 clone, `routed` on a golden-v5h clone with the helpers installed, granted and enabled, plus the brokered GUI drive — exit 0 (which also asserts zero alert beeps and clean teardown of every clone). Each arm is reported by name; a green direct arm alone certifies nothing about the identity every field host runs under. A RED result **blocks the release** until reconciled per [lab/drift-runbook.md](../lab/drift-runbook.md) step 3 — the failure is as often suite drift as a real regression, and either way it is reconciled before the tag, never after. Record the run ids and the per-suite probe counts; they go in the PR body.
...
The host class is a clone of `things-lab-golden-v5h` — the golden that carries the signed helper pair with all four macOS grants already given (HELPGST1). Nothing in this stage runs on the maintainer's machine.
...
- **The runner for this stage is [`lab/scripts/stage5-rc-run.sh`](../../lab/scripts/stage5-rc-run.sh).** It does the provisioning the stage's law fixes — clone `things-lab-golden-v5h`, airgap, pin the clock, ship the RC's `dist` plus the host-built helper bundle, install through the CLI's own path, assert routing — and takes the batch's cells as `GUEST_CELLS` (default [`lab/guest/stage5-cells.sh`](../../lab/guest/stage5-cells.sh), the v0.20.9 batch's 21 plus RAWAX1's two additions; the v0.20.10 batch's 33 are [`lab/guest/stage5-cells-0.20.10.sh`](../../lab/guest/stage5-cells-0.20.10.sh), the v0.20.11 batch's 42 are [`lab/guest/stage5-cells-0.20.11.sh`](../../lab/guest/stage5-cells-0.20.11.sh) and the v0.20.12 batch's 150 are [`lab/guest/stage5-cells-0.20.12.sh`](../../lab/guest/stage5-cells-0.20.12.sh)). Write the batch's cells as a new guest script rather than editing the last one, and keep the previous batch's beside it: a cell list is the audit trail of what a release certified. [`lab/scripts/stage5-ptrdiag.sh`](../../lab/scripts/stage5-ptrdiag.sh) is the companion when a pointer gesture refuses and the question is which of the guard's four legs said so — it asks the SHIPPED guard, generated from the RC's own `dist`, so the answer is never a re-implementation. Two traps both runners close, and any new one must: `node_modules/commander` is resolved by walking UP (an agent worktree has none of its own), and `deputy/build/` is gitignored, so the bundle is built when absent rather than assumed. A third belongs to the CELL scripts, which run under the guest's **bash 3.2**: never write a `{…}` brace expression inside a `$( )` inside a double-quoted string — 3.2 brace-expands the inner text at its commas and hands the shell fragments. Assign the substitution to a variable at statement level instead (v0.20.10's gate run printed three `SyntaxError`s per sidebar move where a display summary belonged; the assertions were unaffected because each reads the database, the trace or the saved envelope). A fourth belongs to the ASSERTIONS: **parse a trace record, never grep it.** `json.dumps` writes `"stop": "placed"` with the space, so v0.20.11's first gate run grepped for the unspaced form and called all fourteen landed drops failures. And a sidebar assertion that reads the group-container plist (the fold's `collapsedAreaUUIDs`) must WAIT for it: Things writes its preferences asynchronously, and three seconds after the drive is not always enough — poll until it drains rather than reading once. A fifth is about CONCURRENCY cells, and v0.20.12's mutation-lock cell paid for it twice: **an observation that costs more than the window it observes cannot be taken in front of the thing being measured.** A sidebar drag lands in ~2.7 s in a clone, so a probe three seconds in reads a released lock (that is not a lock defect, it is a missed window), and `things rescue status` — which reads the lock in its first milliseconds and then censuses the screen — costs ~1.5 s, enough to hand the contending write a composite with 70 ms left to run. Run the observation CONCURRENTLY with the contender, and judge every claim against the holder's own recorded end time so a missed window reports itself as a missed window rather than as a product defect.
...
Versions (package RC, Things app build, macOS, helpers version), the golden the routed arm cloned (`things-lab-golden-v5h`), the exact commands, and each outcome. This paragraph is the audit trail for the gate — a release PR without it has not passed the gate.
...
**Assert the built helper's deployment target, every time, whether or not that diff is empty:**

```sh
bash scripts/build-helpers.sh
otool -l "deputy/build/Things API Helper.app/Contents/MacOS/things-deputy" | grep -A3 LC_BUILD_VERSION
```

`minos` must read **13.0**. `swiftc` with no `-target` defaults the minimum OS to the BUILD HOST's, so this value silently tracks whatever macOS the maintainer last upgraded to, and a helper built too high is refused by dyld on every older Mac with no diagnostic beyond "the deputy is not running (no socket)" — a shipped bundle that is unloadable for most users. `scripts/build-helpers.sh` pins `arm64-apple-macos13.0` (Things' own `LSMinimumSystemVersion` floor), but the pin is a default an environment can override, and the failure is invisible on the machine that builds it. Evidence: [lab/gv5-324-campaign.md](../lab/gv5-324-campaign.md) §5.3.
