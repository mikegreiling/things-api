...
| Clear reminder | 🟡 today/evening in place; 🟡 DATED via `when=today`→re-date BOUNCE (RC01/RC02) | 🟡 DATED via `move … to list "Inbox"` (de-schedules, RC03) — ⛔ no reminder property to clear directly (RC avenue C: −10006) | ✅ **shipped, in-place, incl. DATED** (`todo.clear-dated-reminder`, `things todo clear-reminder`; scf P3b, `set-detail` Reminder Time = `""`) | same-date bare `when=` is STICKY (R20/R21, oddity 2e), but `when=today` (keyword) clears + re-dates to today, so a pure-URL bounce clears (RC01/RC02) — non-atomic, transient Today re-date, CRASHES on repeating items. AS move-to-Inbox drops it by de-scheduling (RC03). **Shortcuts is the only *in-place / schedule-preserving* clear** (startDate untouched, headless) — NOT the only clear. Wired 2026-07-11; capability-gated behind `things setup shortcuts` |
| Set/clear deadline | ✅ | 🟡 | 🧪 | |
