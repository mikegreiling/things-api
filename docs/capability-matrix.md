...
[^ui]: 🟢 = shipped via the **ui write vector** (the fourth vector: Accessibility-API-driven GUI mutations, most-disruptive tier, fail-closed + two-key gated — `ui.enabled` + `--dangerously-drive-gui`). There is no dedicated ui column (these ops exist on no other surface, so they live in the URL/AppleScript/Shortcuts columns as 🚫 and carry the ui note in Notes). **Certification (UIC1, 2026-07-14, Things 3.22.11 / macOS 15.7.7 / DB v26):** the per-op status now has two tiers — `lab-certified` (run end-to-end through the real pipeline in a VM clone, DB deltas asserted) and on-device `certified` (final confirmation on target hardware). **5/7 LAB-CERTIFIED**: pause, resume, make-repeating, reschedule, `todo.convert-to-project`. **2/7 FAILED and stay UNCERTIFIED** — `todo.stop-repeat` (the Stop card opens only via a mouse double-click) and `heading.convert-to-project` (a heading is not selectable via `things:///show`); both blocked by Things list rows exposing no AX/URL selection handle (AXVM1). Certifying required recipe fixes (main-window addressing, select-popup for the frequency dropdown, nested interval field, `action-button-1` confirm buttons). Architecture: [design/ui-vector.md](design/ui-vector.md); certification: [lab/uic1-certification.md](lab/uic1-certification.md) + [lab/ui-certification-runbook.md](lab/ui-certification-runbook.md). Evidence: UIC1-a (+ UI2-a make-repeating, UI2-b reschedule, UI2-c pause/resume, UI2-d convert, UI2-i stop).
...
| Capability | URL scheme | AppleScript | Shortcuts | Notes |
|---|---|---|---|---|
| Create a repeating to-do/project | 🚫 (E13 disproven) | 🚫 (error 302) | 🚫 per docs (no repeat parameters in the actions) | UI-only. Brand-new repeating create (`File → New Repeating To-Do`) is VNC-feasible (UI1 2026-07-12) but is NOT a shipped op. **Make an EXISTING item repeat is 🟢 SHIPPED-UNCERTIFIED via the ui vector** (`todo.make-repeating` / `things todo make-repeating <ref> --frequency <daily\|weekly\|monthly\|yearly> --interval <1-99>`; MCP `make_repeating`; Tier 2 dialog data entry) — Items → Repeat…, driven via Accessibility. **IDENTITY REPLACEMENT** (the original uuid dies; a new template + spawned instance are born) and **IRREVERSIBLE**; result returns the new template uuid via DB diff. Supported rule vocabulary is the v1 subset only: frequency ∈ {daily, weekly, monthly, yearly} + interval 1..99 (no weekday pickers, ends-bounds, dialog reminders, or after-completion/fixed type selection). Fail-closed, two-key gated; **LAB-CERTIFIED (UIC1-a: new template `fu`=256 weekly + spawned instance, original uuid hard-deleted)**. [design/ui-vector.md](design/ui-vector.md) · [lab/uic1-certification.md](lab/uic1-certification.md) |
| Edit / pause / resume / **stop (un-repeat)** a repeat rule | 🚫 | 🚫 | 🚫 per docs | **🟢 SHIPPED-UNCERTIFIED via the ui vector** (four ops, driven via Accessibility): `todo.reschedule-repeat` (`things todo reschedule-repeat <ref> --frequency … --interval …`; Tier 2), `todo.pause-repeat`, `todo.resume-repeat` (`things todo pause-repeat`/`resume-repeat <ref>`; Tier 1), `todo.stop-repeat` (`things todo stop-repeat <ref>`; Tier 1); MCP `reschedule_repeat` + `set_repeat_state {state: pause\|resume\|stop}`. **Reschedule** (Items ▸ Repeat ▸ Reschedule…) mutates the rule bytes IN PLACE, **identity preserved** (fu 256→8 weekly→monthly), but is classified **IRREVERSIBLE** because the minimal v1 vocabulary can't faithfully restore an arbitrary prior rule. **Pause/Resume** (Items ▸ Repeat ▸ Pause/Resume) set/clear `rt1_instanceCreationPaused` — the mutually-inverse **REVERSIBLE** pair (undo pause = resume, and vice-versa). **Stop** (UI2-i, corrects UI2-c's "no Stop") is reachable **only** from the open-card repeat-bar popover (double-click row → click "↻ Repeat every …" → Change…/Pause/**Stop**/Show Latest) — absent from the Items menu and row context menu (menu-parity oversight, oddities §8g); it is a **confirmation-guarded identity-replacement un-repeat**, **IRREVERSIBLE/terminal** (template uuid hard-deleted → NEW plain to-do with `rt1_recurrenceRule` cleared; spawned instance survives; no Resume). Result of stop returns the new plain-to-do uuid via DB diff. Fail-closed, two-key gated. **Certification (UIC1-a): `reschedule-repeat` (`fu` 16→8, identity preserved), `pause-repeat`, `resume-repeat` are LAB-CERTIFIED; `stop-repeat` FAILED and stays UNCERTIFIED** — the Stop popover is card-only and the card opens only via a mouse double-click, which the AX-only vector cannot synthesize (list rows expose no AX/URL handle); it refuses fail-closed. **UNBLOCKED technically (NATIVE1, 2026-07-15):** a synthesized double-click (JXA ObjC bridge → `CGEventPost(kCGHIDEventTap)`, foreground only) at the AX-resolved row DOES open the card and the repeat bar is reachable — so `stop-repeat` is now buildable; whether to build or drop it is Mike's ruling #3 / CRASH1 decision, not a technical blocker. [design/ui-vector.md](design/ui-vector.md) · [lab/uic1-certification.md](lab/uic1-certification.md) · [lab/native1-spike.md](lab/native1-spike.md) |
