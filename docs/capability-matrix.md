...
| Edit / pause / resume a repeat rule | 🚫 | 🚫 | 🚫 per docs | `rt1_instanceCreationPaused` is DB-only today; UI is VNC-drivable (UI2-b/c 2026-07-14): **edit** (Reschedule…) mutates the rule bytes IN PLACE, identity preserved (fu 256→8 weekly→monthly); **Pause** sets `rt1_instanceCreationPaused=1` + clears `rt1_nextInstanceStartDate`, Resume toggles it back. **There is NO "Stop" command** — the Repeat submenu (menu-bar == context-menu) is only Reschedule / Pause↔Resume / Show Latest; end a rule via an "Ends after/on date" bound in Reschedule or by deleting the template. Cannot un-repeat back to a plain to-do via UI. [ui-vector-research.md](lab/ui-vector-research.md) |
