...
| Clear reminder | 🟡 today/evening in place; ✅ DATED via `when=today`→re-date BOUNCE (RC01/RC02) — **wired as the non-repeating fallback** | ⛔ move-to-Inbox is DESTRUCTIVE (strips project/area/heading links + de-schedules — undo cannot restore the container, undo.ts:498–508); NOT a viable reminder clear. No reminder property to clear directly (RC avenue C: −10006) | ✅ **in-place, incl. DATED & REPEATING** (`todo.clear-dated-reminder`, `things todo clear-reminder`; scf P3b, `set-detail` Reminder Time = `""`) | **REVERSIBLE** (undo re-sets the reminder via URL `when=<date>@<time>`, R17/R18 — the repeating clear is irreversible). The `todo.clear-dated-reminder` orchestrator auto-prefers Shortcuts (in place, only path for repeating) and falls back to the URL bounce for NON-REPEATING dated items when the proxy is absent (bounce is non-atomic + transient Today re-date, and CRASHES on repeating items — R09). same-date bare `when=` is STICKY (R20/R21, oddity 2e). `--vector shortcuts\|url-scheme` forces a path. Wired 2026-07-11 |
| Set/clear deadline | ✅ | 🟡 | 🧪 | |
