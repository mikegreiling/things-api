...
[^ui]: 🟢 = shipped via the **ui write vector** (the fourth vector: Accessibility-API-driven GUI mutations, most-disruptive tier, fail-closed + two-key gated — `ui.enabled` + `--dangerously-drive-gui`). There is no dedicated ui column (these ops exist on no other surface, so they live in the URL/AppleScript/Shortcuts columns as 🚫 and carry the ui note in Notes). **Shipped but UNCERTIFIED**: fail-closed, with a per-op certification status flipped to certified by a certification run — a lab operation in a VM clone per Things version (AXVM1 proved Accessibility is grantable in a guest), with a final confirmation on target hardware. Architecture: [design/ui-vector.md](design/ui-vector.md); certification: [lab/ui-certification-runbook.md](lab/ui-certification-runbook.md). Evidence: UI2-a make-repeating, UI2-b reschedule, UI2-c pause/resume, UI2-d convert, UI2-i stop.
