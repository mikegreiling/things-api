...
| Sidebar: areas | 🚫 quiet vectors (O13 + scf2 P6e/f/g + P9b/c) · **🟡 ui-vector FEASIBLE (NATIVE1 spike, unbuilt)** | Quiet-vector paths exhaustively closed 2026-07-09, twice: move-to-location errors (−1700), `set index` read-only (−10006), private reorder no-ops on area uuids in EVERY specifier the sdef admits (named lists, application, area); creation appends at index 0 (the app writes TMArea."index" only on UI drag); no list contains areas. **No longer "the last unautomatable ordering": NATIVE1 (2026-07-15) proved a synthesized mouse drag between two AX-resolved sidebar rows reorders an area — the drag wrote `TMArea."index"` 0 → −448 (sparse/negative, lower sorts higher), confirming that column as the sort key.** Foreground+unlocked only (HID event tap, not PostToPid). Ready for the AXDRAG1 build (scroll edge case needs synthesized scroll-wheel — `AXScrollToVisible` unavailable on rows, −25206). [lab/native1-spike.md](lab/native1-spike.md) |
