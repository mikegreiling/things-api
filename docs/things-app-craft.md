...
"This Evening" (`startBucket=1`) is **inseparably bound to `startDate == the device's current local day`** on every surface — evening is a sub-placement of *Today*, and Today is always the wall-clock day. An evening item whose day has passed does not linger as a stale evening entry: it **rolls back into flat Today** (no "This Evening" section), and a future-dated evening item pins into Today/Anytime a day early rather than waiting in Upcoming. The design consequence is that an evening flag can never accumulate as a wrong-day artifact in the *presentation* — the render always re-derives placement against today. (The underlying `startBucket=1` byte is left in place when the day goes stale rather than cleared — see oddities §9n — but the render-time anchoring means it never *shows* as evening on the wrong day; the honest cost is that a reader keying on the raw column must suppress it exactly as the GUI does.) The same invariant is why no headless surface can write a dated-evening row (`startBucket=1` with a non-today `startDate`) — evening and a non-today date are mutually exclusive by construction (TIMEZ-NODATE). Evidence: [atlas/schema-v26.md](atlas/schema-v26.md) (`startBucket`, "This Evening expires daily"); [reference/timezones.md](reference/timezones.md) §4/§5, [lab/timez-evening-and-zones.md](lab/timez-evening-and-zones.md) Z-ROLL / TIMEZ-NODATE (Things 3.22.12, golden-v2); oddities §9n; [lab/stev1-stale-evening.md](lab/stev1-stale-evening.md) (Things 3.23, golden-v4 — the render is now measured in the AX tree: the stale rows draw at the bottom of Today proper, ABOVE the "This Evening" header, which still appears for the live ones, and the row is byte-identical across a rollover, a view visit, a select-and-scroll and two relaunches — the app re-derives placement rather than migrating data, every single time).
...
### 1e. A repeating series carries no cached container — the spawner reads the template's at MINT time

A template is a factory, so the interesting question about moving one is not "did the FK change?" but "where do its future products land?". Things answers it the cheapest correct way: **nothing about the container is copied into the rule or cached on the series** — the spawner reads the template row's `project`/`area` at the moment it mints, so moving a template between containers is fully forward-effective with no repair pass and no rule rewrite. Measured on BOTH minting paths against the same fixture shape: rolling the clock onto two missed daily slots inserted both occurrences straight into the new project, and the `Create Next Copy` gesture minted its occurrence there too, in each case with the template's `rt1_recurrenceRule` byte-identical by sha256 and only the ordinary cursor advance beside it (TMOV1 §6.2). The whole delta of the move itself is three fields — the template's container FK, its `userModificationDate`, and the two containers' cached leaf counts.

The corollary is the honest one and Things does not pretend otherwise: an occurrence that has ALREADY been materialized is an ordinary row with its own container, so it stays where it was and the series shows up in two places for a while. That is the same thing the GUI does, and it is the right trade — the alternative is a cascade that rewrites rows the user may have deliberately filed elsewhere. It is simply invisible from the move's own result, which is why ours discloses it. Evidence: [lab/tmov1-template-container-move.md](lab/tmov1-template-container-move.md) §6.
...
The payoff is that accepting the defaults is a *correct* action, not a lazy one: committing the dialog untouched after picking only a frequency lands a rule blob equivalent to the one produced by clicking every control by hand (11/11 cells, rule blobs decoded out of SQLite). Our own drive now depends on that: it verifies each pre-filled control by reading it and drives only the residual, and **8/8 traced shapes plus 6/6 dialog states land rule blobs byte-identical to the same drive clicking everything** — which is the derivation being correct often enough to build on, not merely often enough to notice. Two refinements the build had to make to the law as first written: the anchor is `max(scheduled, deadline, today)` rather than `max(scheduled, today)` (the deadline term is what the first bullet above describes, and it has to be IN the expression), and the deadline offset is capped under `after completion` ([oddities §32](things-app-oddities.md)). Evidence: [lab/defaults1-repeat-dialog-defaults.md](lab/defaults1-repeat-dialog-defaults.md) §3–§7, [lab/defaults2-minimal-recipe.md](lab/defaults2-minimal-recipe.md) §3–§5. Things 3.23.

---

### 6n. An after-completion series will not let its occurrences overlap
...
And a third note, which only a failed attack could establish: **the gate is real.** Shipping an unfinished internal feature inside a public binary is normal; shipping it so that an outsider who has fully reverse-engineered the switch still cannot throw it is not. The flag's activation is an `NSUserDefaults` key derived by XOR from a constant sitting in plain sight in the binary — so the obvious assumption, and the one this project made, is that deriving the key is the whole job. It is not. With the key correctly derived and written into the app's own app-group preferences, where it persists across relaunches beside `uriSchemeEnabled`, Things starts and holds zero unix sockets; the same is true of two other defaults-activated flags. Whatever actually admits a feature — most likely an in-app toggle reachable only from a panel gated on a Cultured Code account — is not something a `defaults write` can reach. The obfuscation is a speed bump, as phase 1 said; the thing behind it is a lock.

Evidence: [lab/mcpsrv1-static-recon.md](lab/mcpsrv1-static-recon.md) §2–§4 (MCPSRV1 phase 1, 2026-09-14, static bundle inspection) and [lab/mcpsrv1-guest-probe.md](lab/mcpsrv1-guest-probe.md) §4–§6 (phase 2, 2026-09-14, golden-v5 guest). Things 3.24 build 32400006; byte-identical in 3.23 build 32300036, where it first shipped.
