...
- **No routed lab arm.** The lab certifies direct execution; the routed arm is still the maintainer's-host RC smoke (see the open "Helpers IN THE GUEST" queue item — this campaign is another reason it is worth building). *(CLOSED 2026-09-03, the same day: HELPGST1 built it. `things-lab-golden-v4h` carries the granted helper pair and `lab:regress` now runs the write-layer e2e under both identities plus a brokered GUI drive — [helpgst1-helpers-in-guest.md](helpgst1-helpers-in-guest.md). The gap this bullet records was real when it was written; the note is here so the pointer does not dangle.)*
