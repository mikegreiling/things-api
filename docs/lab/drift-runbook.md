...
3. **Re-run the full regression** against the new golden: `npm run lab:regress`. Any verdict/tier delta = the update moved the write surface; reconcile suite expectations deliberately, updating the results docs and capability notes with evidence. **Then WALK THE REGISTER** ([reference/assumption-register.md](../reference/assumption-register.md)): confirm every law's recurring lock still passes, and re-verify by hand — against each row's campaign doc — any law whose lock is a GAP (the schema fingerprint cannot see a BEHAVIORAL change; the register is what catches an insertion/preservation/refusal law that moved with zero schema delta). **In the SAME pass, amend the register's *Confirmed under* column:** append the new golden's Things version to every law row whose lock ran green; a law whose lock FAILED keeps its stale version list untouched (that stale list IS the drift signal) until it is re-probed and reconciled, then the new version is appended. Do NOT amend the campaign/evidence docs — those are immutable snapshots stamped with the version they were observed under; the register is the living version ledger.
4. **Ship the baseline.** Regenerate the fixture DDL snapshot from the new schema, add `src/db/baselines/db-v<N>.ts` (databaseVersion, fingerprint, known app versions), release. **Also record the newly-certified Things version** so the behavioral-drift notice tracks it: `things config set certified-app-version <X>` (the version the register was just re-walked AND *Confirmed under*-amended against in step 3 — the two must agree: `certified-app-version` names the same Things version now appended as the freshest *Confirmed under* entry across the green laws). `things doctor` then compares the installed vs certified app version and emits a passive, non-blocking notice on a future mismatch, pointing back here (the schema fingerprint stays green across a behavioral-only update, so this notice is the only signal that the laws need re-walking).
