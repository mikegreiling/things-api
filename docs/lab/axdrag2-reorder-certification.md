...
**Verdict (2026-07-15):** the production **`area.reorder-sidebar`** op is **LAB-CERTIFIED** end-to-end through the shipped CLI (`things area reorder-sidebar … --dangerously-drive-gui`) against **Things 3.22.11 / macOS 15.7.7 / DB v26**: rung-1 single drags (both directions), the rung-3 multi-hop floor (3 hops + final drag on a shrunken viewport), to-first/to-last, a verified undo round-trip, and the fail-closed negatives — all DB-asserted with the `TMArea` count and every task's area assignment invariant. **Rung 2 (scroll-while-held) — SHELVED by decision (Mike, 2026-07-15).** It is BUILT and probe-certified but ships DISABLED, and the production ladder stays **rung 1 + rung 3 permanently for now** (held-scroll stays behind `THINGS_UI_DRAG_LADDER=held-scroll` for lab work). Reasoning: its long-travel form is the strongest trigger of the oddities §9 app bug — after drag+scroll churn the sidebar's **AX mirror drops or blanks row elements until Things is relaunched** — and that blanking **blinds the closed-loop verification the doctrine requires** (the driver can't assert what it can't read); rung 3's per-hop released-drag DB asserts sidestep it. **Revisit** only if Cultured Code fixes the AX-mirror bug, a non-relaunch heal is found, or a certified travel cap with real margin below the blanking threshold lands. **`tag.reorder` — ABANDONED BY DECISION (Mike, 2026-07-15), not merely blocked**: risky/potentially destructive for little utility — the chain breaks at its first link (tag rows expose no names and the DB→row mapping is unverifiable whenever `TMTag."index"` ties exist, which they always do for never-dragged tags, so a mis-aim could silently re-parent with no reliable recovery addressing). The TAGORD1 probes below settled the ordering semantics for KNOWLEDGE only; they do not reopen the build.
