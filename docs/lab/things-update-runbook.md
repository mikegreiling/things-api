...
- The lab-certified version list lives in the golden metadata; the current certified app is **Things 3.23** (golden-v4, build 32300036, `Meta.databaseVersion` 27) on macOS 15.7.7, certified 2026-08-22. See [golden-v4-metadata.json](golden-v4-metadata.json) + [gv4-323-certification.md](gv4-323-certification.md). **Things 3.22.11–3.22.14 stay certified** (golden-v3 is the retained fallback and the arm every version-conditional suite expectation is checked against). The in-place DRIFT-1 swap path (clone the prior golden, `ditto -xk` the new app zip over `/Applications/Things3.app` after `rm -rf`-ing the old bundle, warm-up, re-fingerprint) is the routine recipe when a point release is schema-identical — and produced golden-v4 even though 3.23 DID migrate the database, because the migration is index-only in DDL terms.
- **3.23 carries a standing residue, and the certification is honest about it.** Certifying an app version does NOT mean every law was re-confirmed under it. Two classes never can be: (a) laws SUSPENDED by the update — on 3.23 that is every ordering law resting on `_private_experimental_ reorder to dos in`, which the app now accepts and ignores (their suite rows are green only because they were reconciled to assert that inertness, which is what makes them the behavioral canary); (b) laws whose only lock is unit/engine/evidence-level or needs a rig `lab:regress` cannot host (a clock roll, a TZ shift, a framebuffer, a live Things Cloud account). Both classes are enumerated per-row in the [assumption register](../reference/assumption-register.md)'s 3.23 audit block. Read that block before treating "certified" as "everything re-verified"; 3.22.14 shipped with the same shape of residue, unenumerated.
- **`things config set certified-app-version <version>` is the MAINTAINER's action on the live host, never a lab session's.** It stamps the operator's own config and silences `doctor`'s passive behavioral-drift notice, so it waits on a human reviewing the certification campaign — the lab certifies the golden, the operator accepts it.
