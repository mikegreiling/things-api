...
35 cases across the 21 covered ops. After the **clone leg ran (2026-07-22, run `simfid-20260722-200229`)** and its six CRUD rows flipped to fresh clone captures, the current standing is **29 MATCH · 6 TOLERATED · 0 DIVERGENT · 0 replay-error** (the initial host-only run was 30 MATCH · 5 TOLERATED — `todo-reopen` moved MATCH → TOLERATED under fresh capture, see the finding below). Full artifacts (gitignored): `lab/artifacts/simfid-<stamp>/` (per-case `simNorm`/`golden`/`verdict`, `results.md`, `summary.json`).
...
| `todo-reopen` | `todo.reopen` | crud | **TOLERATED(wallclock-bucket)** ⚠️ verdict changed vs evidence run (was MATCH) — see finding below | **clone-drive (simfid-20260722-200229)** |
...
- **The 6 TOLERATED verdicts.** Five are the two headline recurrence nondeterminisms, and their presence is the point: they prove the tolerances FIRE (present-or-absent both pass) rather than mis-reporting a bug. `instance-next-sentinel` fires wherever a top-level instance is minted (fixed to-do/project + subtree cases); `rt1-child-backlink` additionally fires on the subtree cases (one instance-side child modelled as stamped, per RSIM-S's nondeterministic draw). The **sixth is new to the clone leg**: `wallclock-bucket` now fires on `todo-reopen` against the fresh capture — but here the tolerance is MASKING a real sim-side fixture gap (stopDate not cleared because the fixture never set one), which is the finding above, not a proven nondeterminism. That is a case FOR tightening the tolerance, not evidence it behaved.
- **What the clone leg now proves (and where it caught something).** For the six headless CRUD cases the app deltas are a fresh independent capture (`clone-drive`), so this leg has real DIVERGENT-detection power for them — and it exercised it: it surfaced the `todo-reopen` stopDate mismatch that the banked-evidence run could not (golden == modeled there). Five of the six corroborate the model exactly; the sixth is the finding. The remaining CRUD rows (`suite-evidence`) and the recurrence/subtree family (`rsim-evidence`) are still banked-derived — an UNDOCUMENTED simulator divergence there would not surface until they too are clone-captured (CRUD: follow-up 1; recurrence: needs the AX-capable driver). The unit tests independently prove DIVERGENT detection on synthetic deltas.

## Follow-up worklist (implied by the run + the tolerances)

1. **Run the clone leg** (`bash lab/scripts/simfid.sh`) — **DONE for the starter manifest (2026-07-22, run `simfid-20260722-200229`; disk freed to ~20 GiB).** The six pre-state-immune CRUD cases are now fresh `clone-drive` captures (5 MATCH corroborating the model + 1 finding: `todo-reopen`, above). It also forced the self-contained-guest-node harness fix (see the app-side-provenance note). **Remaining under this item:** (1a) **the `todo-reopen` fixture/tolerance follow-up** — seed `stopDate` on the completed fixture so the sim reproduces a real completed pre-state, and consider tightening `wallclock-bucket` so a present-vs-`null` stopDate difference reads DIVERGENT (both deferred here per this PR's scope); (1b) **expand `clone-manifest.json`** beyond the starter set (the CRUD cases with subtler pre-states: inbox-promotion start values, tag/checklist joins), confirming CLI seeding case-by-case. (The recurrence/subtree family — incl. the two preserve laws — stays out of the headless manifest; it needs an AX-capable driver, see the clone-leg scope note above.)
