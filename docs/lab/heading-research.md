...
- ~~Whether the UI renders an archived heading's still-open children anywhere odd (needs eyes on a VM screen).~~ — **closed by HEADARC (2026-08-03, [headarc-archived-heading-moves.md](headarc-archived-heading-moves.md)):** the state does not arise on the headless surfaces — landing (or reopening) an open child under an archived heading REOPENS the heading (`status 3→0` + `stopDate→NULL`, heading-only, byte-identical to the AS un-archive), so an archived heading never owns an open child to render oddly. ~~Residual (unprobed): a PLOG1-style trash-restore (Put Back a trashed child) might reproduce it for a heading as it does for a project (oddities §6¾).~~ — **closed by HEADARC2 (2026-08-03, [headarc2-residual-captures.md](headarc2-residual-captures.md) ARM C):** it DOES — GUI Put Back of a trashed open child restores it in place under an archived heading WITHOUT reopening (heading stays `status=3`), reproducing the odd state (the §6¾ heading analog); the open child is then invisible to every actionable view, reachable only via "Show N logged items". Reachable ONLY via GUI Put Back — never a headless surface we ship (`todo.restore` un-traps to the Inbox). HEADARC2 also captured the archived-heading rendering the move campaign could not (arm A: archived heading = a grouped logged-section header; arm B: an open heading's swept child carries a muted heading sublabel) — macOS matches the maintainer's iOS recollection.
