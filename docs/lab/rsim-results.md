...
**SIMFID:** this makes the to-do source-fate a MODELLABLE branch, not merely a tolerance — `applyMakeRepeatingFixed` (to-do) may preserve-source iff the pre-read source has a non-NULL `deadline`, else delete. (The bench simulator's always-delete stays correct for bare/checklist bench items; the deadline branch is the added fidelity if repeat-with-deadline corpora are introduced.) Untested edge: a deadline COMBINED with a checklist/notes/tag — B3 (all four) preserved, consistent with deadline dominating; no cell here sets deadline alongside another axis, so "deadline overrides" is inferred, not proven. **[LANDED 2026-07-22, branch `mg/sim-source-fate`: `applyMakeRepeatingFixed` now relinks a deadline-carrying to-do source as the instance (mints only the template; source keeps its own deadline, template drops it). SIMFID case `todo-make-repeating-deadline-preserve` MATCHes; unit test in write-simulator.test.ts (deadline preserve + bare delete control).]**
...
**SIMFID:** `applyMakeRepeatingFixed` (project) must preserve-source when the pre-read subtree has a nested repeater **OR** no open child (all children `status IN (2,3)`), else delete — extends §RSIM-R simulator verdict #1 (the nested-repeater-only preserve condition was incomplete). **[LANDED 2026-07-22, branch `mg/sim-source-fate`: the fixed-project preserve predicate is now `nested-repeater OR all-children-terminal`; the shared preserve helper relinks the source as the instance (children ride along unchanged, `childrenReplaced=0`) and mints only the template. SIMFID case `project-make-repeating-terminal-children-preserve` MATCHes; unit test adds the terminal-children preserve + open-child delete control.]**
