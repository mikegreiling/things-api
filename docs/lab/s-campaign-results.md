...
## Parked re-validation (tart 2.34.0 golden, 2026-07-22, PR mg/parked-probes)

The parked-probe campaign re-ran the P-series on a fresh `parked-probes-lab` clone under **tart 2.34.0** (golden-smoke PASS — no bring-up break; see probe-backlog §C tooling note) to confirm no drift, plus closed the genuinely-open reminderTime bit-layout residual (schema atlas OQ1). Every verdict matched scf2/scf3 **exactly** — no drift on the new hypervisor. Environment: Things **3.22.11 / macOS 15.7.7 / DB v26 / clock 2026-07-05 12:00 / tart 2.34.0**. Script: [`lab/scripts/research-parked.sh`](../../lab/scripts/research-parked.sh); raw `report.txt` + `snaps/` + per-probe `drive-*.log` under `lab/artifacts/parked-probes-lab/`.

| Probe | Case | Verdict (2026-07-22) |
|---|---|---|
| P4 (comp) | AppleScript `set completion date` — **arith AND date-literal spellings** | **WORKS** (reconfirmed + NEW spelling) — completed fixture (`status=3`, stopDate=1783253128); arith `(current date) − 200 days` → stopDate **1765973139**; then date-literal `date "1/15/2025"` → **1736899200** (= 2025-01-15). The **`date "M/D/YYYY"` literal spelling now confirmed working** alongside the scf2 arith spelling. |
| P4a (comp) | Shortcuts `set-detail` Completion Date | **DEAD** (reconfirmed) — `2025-01-15`, `1/15/2025`, `January 15, 2025` all exit-0 NO-OP on a verified-completed fixture. |
| P4c (comp) | URL `update?completion-date=` (+token) | **DEAD** (reconfirmed) — `completion-date=2025-01-15` zero-delta (oddity 2g). |
| P4 (crea) | AppleScript `set creation date` — arith + date-literal | **WORKS** (reconfirmed + NEW spelling) — arith → creationDate **1748693152**; date-literal `date "6/1/2024"` → **1717200000** (= 2024-06-01). |
| P4a/c (crea) | Shortcuts + URL creation-date backdating | **DEAD** (reconfirmed) — Shortcuts (`2024-06-01`, `6/1/2024`) and URL `creation-date=2024-06-01` both zero-delta. AppleScript remains the ONLY backdating surface. |
| P3a | `set-detail` Reminder Time SET, format sweep | **DEAD** (reconfirmed) — `2:30 PM`, `14:30`, `7/5/2026 2:30 PM` all NULL→NULL on a `when=today` fixture. set-detail can only CLEAR (P3b). |
| **P3a BITS** | **reminderTime bit-layout (schema atlas OQ1)** | **CONFIRMED `reminderTime = hour<<26 \| minute<<20`** 🎉 — 4 fresh dated-reminder fixtures created via URL `when=@HH:MM`, all MATCH: 09:00→**603979776** (9<<26), 14:30→**970981376** (14<<26\|30<<20), 00:15→**15728640** (15<<20), 18:07→**1215299584** (18<<26\|7<<20). Independent re-derivation of the R-suite formula; **closes schema-v26 OQ1** (Encodings table de-TBD'd). |
| P2b | `set-detail` Parent (text uuid) on a TO-DO | **DESTRUCTIVE DETACH** (reconfirmed) — to-do's `project` `8hQgsS51…`(ProjA) → **NULL** (detached, NOT moved to target ProjB — oddity 5l). |
| P6a | AppleScript `move project id X to before project id Y` | **DEAD** (reconfirmed) — error **−1700** ("Can't make before project id … into type list"). |
| P6b | `set index of project`/`set index of area` to 0 | **DEAD** (reconfirmed) — both error **−10006** (read-only). |
| P6c | private reorder in `list "Anytime"` with AREA uuids | **NO-OP** (reconfirmed) — area index 0→0, zero delta. |
| P6d | private reorder in `list "Someday"` with top-level PROJECT uuids | **WORKS** (reconfirmed P6h) — P6-Some1 index 0 → **−1232** (CHANGED). Someday project reorder is the live native scope. |
| P6e | sdef private-command inventory (grep the bundle's `Things.sdef`) | **exactly ONE reorder verb** (reconfirmed) — `_private_experimental_ reorder to dos in` (+ its read siblings `current list json` / `json` / `ReorderToDosIn`). No hidden sidebar spelling. |

**Net:** zero verdict drift from scf2/scf3 on tart 2.34.0; backdating stays AppleScript-only (now with the date-literal spelling banked); reminder SET, to-do re-parent, and sidebar/area ordering stay dead; Someday reorder stays live; **schema OQ1 (reminderTime layout) closed.** No capability-matrix feasibility change.

## UI-vector campaign (UI1) — §E½ feasibility + deadline-less-repeat discriminator (2026-07-12, PR mg/ui-vector-probe)

One `--vnc-experimental` clone, autonomous. Reused the SX6 VNC synthetic-input mechanics. **Accessibility is NOT granted in the golden** (`osascript` "System Events" → error −1719 "not allowed assistive access", ~~cannot be granted without disabling SIP~~ — **CORRECTED by AXVM1 2026-07-14: it CAN be granted via the user-path System Settings toggle with SIP enabled; "not granted" ≠ "not grantable"**), so this run had **no AppleScript UI-scripting path available** — every menu click, dialog interaction, and keystroke is VNC (hardware HID, no TCC). (With the grant, AX element press becomes a second, lower-disruption path — see [axvm1-accessibility.md](axvm1-accessibility.md).) VNC framebuffer = the golden's physical 2048×1536; layout is deterministic per resolution. Run script: [`lab/scripts/research-ui1.sh`](../../lab/scripts/research-ui1.sh); screenshots (`00`–`14`) + `final.sqlite` + `discriminator-evidence.txt` under `lab/artifacts/things-run-ui1-explore-20260712-180724/`.
...
**Follow-up campaign UI2 (2026-07-14):** the UI-only repeat-management + conversion operations UI1 identified, plus the frontmost/focus-steal disruption profile, moved to their own home — [ui-vector-research.md](ui-vector-research.md). Headline finds: making an existing item repeat is an **identity replacement** (old uuid dies, new template + instance spawn), editing a rule **preserves** identity, Pause = `rt1_instanceCreationPaused`, and Convert to Project (to-do AND heading) is identity replacement. (UI2 also concluded "there is no Stop command" — **falsified 2026-07-14 by UI2-i**: a **Stop** exists in the open-card repeat-bar popover only; it is an identity-replacement un-repeat. See [ui-vector-research.md](ui-vector-research.md) §UI2-i, `lab/scripts/research-ui2i.sh`.) Script [`lab/scripts/research-ui2.sh`](../../lab/scripts/research-ui2.sh).
