...
**RESOLVED 2026-07-14 (AXVM1) — AX-under-lock IS probeable in a VM, and it WORKS.** Accessibility was merely *never granted* in the golden, not SIP-blocked: a one-time user-path TCC toggle (System Settings → `tccd`, SIP stays enabled) grants it, and then a System Events press of **Items ▸ Repeat ▸ Pause** — addressed purely by element name over SSH — succeeded under a genuine `sysadminctl -screenLock immediate` lock, flipping `rt1_instanceCreationPaused` 0→1. This is the exact counterpoint to arm (f): the VNC *coordinate* click hits the lock screen, but the AX *element* press does not care about presentation state. AX element press also needs no frontmost app and steals no focus (paused a template with Finder frontmost; Finder stayed frontmost), and the grant persists across reboot. Full write-up + the VNC-vs-AX comparison: [axvm1-accessibility.md](axvm1-accessibility.md).
...
**Second-account confirmation (2026-07-14, while shipping the `doctor` sync-health section).** On the maintainer's real, long-lived account the account-specific key `GryCJ44xPcJG6go5KeTZp1` was **absent** — the value-based **nearest-to-now** fallback is what resolved the last-attempt timestamp. This confirms the one-account caveat above (the opaque key is account-specific, not universal) and validates shipping the fallback as the robust primary path rather than a mere backstop. The shipped reader (`src/sync-health.ts`) tries the known key first, then falls back to the `BSSyncronyMetadata` bplist-double nearest to now (excluding the ~now+31yr lease sentinel).
