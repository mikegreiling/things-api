...
**Repeating-item compatibility — RESOLVED 2026-07-12 (round-3 RCLEAR probe, oddities §8b).** The residual doubt is settled, and the answer overturns the "Shortcuts is the in-place clear for repeating items" hypothesis: **a repeating template has no clearable reminder at all on any automation surface.** Its reminder is a repeat-RULE property, NOT a `reminderTime` column value — setting "Add reminders 12:00 PM" in the GUI repeat editor persists in the editor but writes NOTHING to `reminderTime` (template or pre-spawned instances) and adds no time-key to the decoded rule. Consequently, on a repeating template:
- **URL `when=` bounce — CRASHES** (avenue B, §1/§7, U12/R09/P14-A4), as before.
- **AppleScript `move … to list "Inbox"` (avenue A) — CLEAN REFUSAL, error 301, NO crash** (now probed: the AS guard rejects de-scheduling a repeating item — the very guard the URL path lacks; PID stable, rule intact).
- **Shortcuts `set-detail Reminder Time=""` — SAFE no-op** (exit 0, no crash, rule intact) — there is simply no `reminderTime` to clear.

So there is **no in-place automation clear** for a repeating template's dated reminder — it can only be edited in the app (rule-level, UI-only). `todo.clear-dated-reminder`'s repeating refusal is correct and stays; the `H-NO-REMINDER` guard (reminderTime NULL) already blocks the op on a template before any surface is attempted.
...
## Implications wired (2026-07-11, PR #73)

RC01/RC02 are now shipped as a real fallback. `todo.clear-dated-reminder` grew a URL-bounce path (`src/write/clear-reminder.ts`): a compound of two verified `todo.update` legs (`when=today` → `when=<original date>`) wrapped in a txn with a single undoable SUMMARY audit record — the reorder/heading compound-runner pattern. The orchestrator auto-prefers the atomic Shortcuts `set-detail` when the proxy is installed and falls back to the bounce for NON-REPEATING dated items when it is not; a repeating template with the proxy absent is BLOCKED with the `things setup shortcuts` remediation (never bounced — R09 crash), and `--vector shortcuts|url-scheme` forces a path. Leg-2 failure surfaces the exact residual state (reminder cleared, item stranded on Today) instead of pretending atomicity. RC01/RC02 also made the clear REVERSIBLE: `things undo` re-attaches the reminder via the URL set path (R17/R18) against the item's CURRENT schedule, guarded by a new precondition check (current reminder must still be null, else refuse). The stale "IRREVERSIBLE — dead on every surface (scf2 P3a)" undo reason is gone.
