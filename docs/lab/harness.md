...
## The lab escapes (`THINGS_API_UI_DIRECT=1`, `THINGS_API_WRITE_DIRECT=1`)

A shipped host reaches Things only through grants a golden clone cannot hold. Since the permissions doctrine's [Article IV](../design/permissions-doctrine.md) the Things **window** may be driven only through the helper pair — the signed identities that hold Accessibility + Automation → System Events — and since Wave A's write gate an **Apple Event** may be sent only on a recorded Automation grant, which is keyed to a bundle id. A clone has no helper bundle, nobody at the screen to answer a consent dialog, and no bundle id at all in an sshd-descended shell. What it *does* have is the **AXVM1 layer** — an in-guest Accessibility + Automation grant on the runner's own sshd-descended processes ([axvm1-accessibility.md](axvm1-accessibility.md)) — which is exactly the direct access the doctrine forbids on a user's Mac.

Two env vars are the documented escapes for that one situation. Neither is **consumer surface**: nothing in the CLI/MCP copy mentions them, no consumer path sets them, and each is read in exactly one place — the capability function for the vector it covers (`src/capability.ts`). Both spell "on" as the literal `1`; anything else is off.

| Escape | Vector | What it does, and what it does not |
|---|---|---|
| `THINGS_API_UI_DIRECT=1` | ui (GUI-driving) | Read by `uiCapability` alone. Restores direct UI-vector availability. It does **not** bypass `ui-enabled`, so a clone still runs `things config set ui-enabled true` as it always did |
| `THINGS_API_WRITE_DIRECT=1` | applescript (Apple Events) | Read by `writeCapability` alone, and **only on the bundle-id-less path**. Without it `writeCapability` reads `direct-unknown` in every clone, and every AppleScript-vector verb — plus every composite with an AppleScript leg, `make-repeating` and `add-repeating` included — refuses `blocked:environment`. A host that HAS a bundle id is answered from its TCC row exactly as before, so the escape can never mask a recorded refusal (`direct-denied`) or invent a grant for a real terminal |

Reads (`direct-fda`), the URL scheme and Shortcuts were never gated by either and need no escape.

Where they are exported:

| Site | What it covers |
|---|---|
| `lab/guest/e2e-write-smoke.sh` | the guest e2e bundle — **both** escapes, for every `things` call the write-layer smoke makes |
| `lab/scripts/env.sh` (`$LAB_UI_DIRECT`, `$LAB_WRITE_DIRECT`, `$LAB_DIRECT` for both) | the prefix every **bash campaign driver** must put in front of a guest CLI invocation that exercises the matching vector: `lab_ssh "$IP" "$LAB_DIRECT $CLI …"` |

A driver that forgets one gets a clean `blocked` (exit 4) naming the remediation — `things helpers setup --gui` for the ui vector, `things setup` / `things helpers setup` for the AppleScript one — never a hung dialog. The refusal is the fail-closed outcome, so a forgotten prefix shows up as a red probe rather than a wedged VM.

Unit and simulator suites are unaffected and must NOT use either: they exercise these vectors with fake vectors, which are ungated because each gate keys on the vector's DECLARATION (`drivesGui`, `sendsAppleEvents`) rather than on its id — a fake declares nothing and drives nothing.

> **The ui escape was VERIFIED end to end 2026-08-24 (CNC1, golden-v4).** It does what it claims: with `THINGS_API_UI_DIRECT=1` a shipped pure-ui op (`todo pause-repeat`) drives in a clone and the pause lands in the database; without it the same call is a clean **exit 4** naming `things helpers setup --gui`, with nothing driven and no dialog. The same campaign is what discovered the second gate: the AppleScript vector was blocked unconditionally in every clone, `doctor` in-guest read `applescript direct-unknown`, and campaign drivers had to build repeating fixtures the REPX2/REPX3 way — a URL-scheme add plus a direct AX Repeat-dialog drive. That is what `THINGS_API_WRITE_DIRECT=1` closes; the workaround is no longer required, though the evidence written under it stands. See [cnc1-template-mutations.md](cnc1-template-mutations.md) §9.
