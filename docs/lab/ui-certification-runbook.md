...
> **The in-VM suite has RUN (UIC1, 2026-07-14; extended UIC3, 2026-07-15):** [uic1-certification.md](uic1-certification.md) certified the to-do ops `lab-certified` against Things 3.22.11; [uic3-build-certification.md](uic3-build-certification.md) then added the three repeating-**project** ops (`project.reschedule-repeat`/`pause-repeat`/`resume-repeat`, the mouse-synthesis hybrid) — now **8/9 lab-certified**, with only `heading.convert-to-project` failing (no `things:///show` handle for a heading). `todo.stop-repeat` was **dropped**. `lab/scripts/research-uic1.sh` and `lab/scripts/research-uic3.sh` are the executable versions of §0–§4 below. The manifest carries a `lab-certified` tier; this runbook's remaining job is the **on-hardware `certified` confirmation** (§5) that appends the hardware evidence id — and re-certification after a Things update.
...
Keep the session **unlocked** for the whole sitting (LOCK1): a locked session presents only the lock screen. Since LOCKSCR1 ([#732](https://github.com/mikegreiling/things-api/issues/732)) the drive no longer merely fails on a locked screen — it **refuses on its first hop**, `blocked` / exit 4, in about a third of a second, naming the lock: *"Refused to drive the Things window: the screen is locked, so no window can be read or clicked."* `things doctor --ui-state` prints the reading as its `session:` row — check it first when a whole batch of drives refuses at once. Menu-only ops (pause / resume) are deliberately ungated and still work under lock (AXVM1-d).

**A running SCREEN SAVER is now nudged rather than refused (LOCKSCR2).** The window server sets `CGSSessionScreenIsLocked` for a bare saver too, so a saver used to take the lock refusal; a drive that meets one now posts a single left-Shift, re-reads the session, and proceeds if it cleared — disclosing *"the screen saver was up and was dismissed to run this"* on the result. Two things follow for a sitting. **(a)** A saver that fires mid-sitting will be dismissed by the next drive, and the screen stays awake afterwards; a cell whose subject is the saver must therefore set the saver state deliberately rather than inheriting it. **(b)** If the Mac requires a password after the saver, the nudge changes nothing and the drive refuses with *"the screen saver is up and did not clear when the Mac was nudged, so the Mac is asking for a password"* — the same 4 s every time, because that is the nudge's re-read budget. Disabling the saver for the sitting is still the simplest posture.

**A CLOSED Things window no longer stops an `area reorder` (LOCKSCR2).** With the session proven unlocked, the drive runs Things' own `reopen`, re-censuses, and carries on — and **leaves the window open**, disclosing *"Things had no open window, so one was reopened to run this — it was left open"*. A sitting that counts windows before and after a reorder should expect one more at the end than it started with when it started with none. The dialog-class ops are unchanged: they still refuse with SESSGATE's *"Things has no open window"*.
