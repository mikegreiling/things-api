...
- Remaining gaps: heading-scoped ordering (unautomatable, O06); sidebar AREAS among areas (exhaustively dead — O13 + scf2 P6e/f/g, the LAST unautomatable ordering); checklist-item order (likely unautomatable). SOLVED 2026-07-09 and SHIPPED: heading reorder (`--scope headings`, scf P1), someday loose to-dos (`--scope someday`, two-call anchor protocol P8b), and **top-level sidebar PROJECT order via Mike's bounce insight** (`--scope projects`, when=someday→anytime round-trips front-insert — P7c/P7d/P8e; native sidebar writes stay dead per scf2 P6). Anytime loose-to-do ordering moved in probes but its convention is unlocked (P7b/P8d) — unshipped.
