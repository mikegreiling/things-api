...
- **things-reader** (`reader/`, nested at `Contents/Helpers/things-reader.app`, SANDBOXED) — database/file reads through a durable security-scoped bookmark to the Things folder, minted once by `things helpers setup` (SANDBOX1, [docs/lab/sandbox1-scoped-reader.md](../docs/lab/sandbox1-scoped-reader.md)). Serves `sql`/`read-file`/`locate` on a socket **launchd** owns at `~/.local/state/things-api/reader/reader.sock` — declared in its LaunchAgent's `Sockets` key and handed over as a listening fd at activation, because a sandboxed process can only bind inside its own container and a rendezvous in there costs every CLIENT a cross-app-container consent (docs/design/agent-daemon.md § The rendezvous). Ships as a signed minimal .app because amfid refuses ad-hoc/self-signed roots on sandboxed code and secinit refuses bare executables. Its bundle identifier keys the user's bookmark grant and must never change.
...
- Deputy socket, token, and logs live in a 0700 state dir (`~/.local/state/things-api/deputy/`); socket and token are 0600. The deputy binds its own socket — it is unsandboxed, so its rendezvous was never in a container.
- The reader's rendezvous is the sibling 0700 dir `~/.local/state/things-api/reader/`: a 0600 socket launchd binds there, and a 0600 token `things helpers setup` mints, hands to clients as a file, and hands to the reader through its LaunchAgent's `EnvironmentVariables` (which is why that plist is 0600 too). Only the security-scoped bookmark and the audit log stay in the sandbox container.
- Every connection is peer-checked (same UID); every request must carry the token's value — a sandboxed process that cannot read the token cannot use a helper.
